...
function AbstractFieldMission:loadFromXMLFile(xmlFile, key)
	...
	if self.field == nil then
		g_logManager:xmlError("missions.xml", "Mission field is not available.")
...
function AbstractFieldMission:writeStream(streamId)
	AbstractFieldMission:superClass().writeStream(self, streamId)
...
function AbstractFieldMission:readStream(streamId)
	AbstractFieldMission:superClass().readStream(self, streamId)
...
function AbstractFieldMission:init(field, sprayFactor, fieldSpraySet, fieldPlowFactor, fieldState, growthState, limeFactor, weedFactor, weedState)
...
function AbstractFieldMission:update(dt)
	...
	if self.vehicleIndexToLoadNext ~= nil and self.lastVehicleIndexToLoad == self.vehicleIndexToLoadNext then
		...
		if self.vehicleIndexToLoadNext <= table.getn(self.vehiclesToLoad) and not self:loadNextVehicle() then
			print("ERROR: Failed to load all vehicles: no space at spawn point")
...
function AbstractFieldMission:resetField()
	for i = 1, table.getn(self.field.maxFieldStatusPartitions) do
		g_fieldManager:setFieldPartitionStatus(self.field, self.field.maxFieldStatusPartitions, i, self.field.fruitType, self.fieldState, self.growthState, self.sprayFactor * g_currentMission.sprayLevelMaxValue, self.fieldSpraySet, self.fieldPlowFactor * g_currentMission.plowCounterMaxValue, self.weedState, self.limeFactor * g_currentMission.limeCounterMaxValue)
	end
end
...
function AbstractFieldMission:onVehicleReset(oldVehicle, newVehicle)
	if oldVehicle.activeMissionId == self.activeMissionId then
		...
		ListUtil.removeElementFromList(self.vehicles, oldVehicle)
...
function AbstractFieldMission:calculateReward()
	...
	local reimbursementPerHa = self.reimbursementPerHa

	if self.reimbursementPerDifficulty then
		local factor = 1.4 - 0.1 * g_currentMission.missionInfo.economicDifficulty
		reimbursementPerHa = reimbursementPerHa * factor
	end

	local difficultyMultiplier = 1.3 - 0.1 * g_currentMission.missionInfo.economicDifficulty

	return fruitMultiplier * self.rewardPerHa * self.field.fieldArea * difficultyMultiplier + reimbursementPerHa * self.field.fieldArea
end
...
function AbstractFieldMission:loadNextVehicle()
	...
	g_currentMission:loadVehicle(filename, x, nil, z, yOffset, yRot, true, 0, Vehicle.PROPERTY_STATE_MISSION, self.farmId, info.configurations, nil, self.loadNextVehicleCallback, self, {
...
function AbstractFieldMission:getMaxCutLiters()
	...
	return fruitDesc.literPerSqm * area * 10000
end
...
function AbstractFieldMission:isSpawnSpaceAvailable()
	...
	for _, v in ipairs(self.vehiclesToLoad) do
		...
		local sizeWidth, sizeLength, widthOffset, lengthOffset = StoreItemUtil.getSizeValues(v.filename, "vehicle", storeItem.rotation, v.configurations)
		local x, y, z, place, width, _ = PlacementUtil.getPlace(places, sizeWidth, sizeLength, widthOffset, lengthOffset, usedPlaces)
...
function AbstractFieldMission:createMapMarkerAtSellingStation(sellingStation)
	local x, _, z = getWorldTranslation(sellingStation.owningPlaceable.nodeId)
	...
	local mapHotspot = MapHotspot:new("missionHotspot", MapHotspot.CATEGORY_MISSION)
