...
function TransportMission:loadFromXMLFile(xmlFile, key)
	...
	if self.status == AbstractMission.STATUS_RUNNING then
		...
		while true do
			...
			local x, y, z = unpack(StringUtil.getVectorNFromString(getXMLString(xmlFile, objectKey .. "#translation"), 3))
			local rx, ry, rz = unpack(StringUtil.getVectorNFromString(getXMLString(xmlFile, objectKey .. "#rotation"), 3))
...
function TransportMission:writeStream(streamId)
	TransportMission:superClass().writeStream(self, streamId)
...
function TransportMission:readStream(streamId)
	TransportMission:superClass().readStream(self, streamId)
...
function TransportMission:init(args)
	...
	local tMission = ListUtil.getRandomElement(g_missionManager.transportMissions)
	local pickup, dropoff = nil

	for i = 1, table.getn(tMission.pickupTriggers) + 1 do
		local item = ListUtil.getRandomElement(tMission.pickupTriggers)
	...
	for i = 1, table.getn(tMission.dropoffTriggers) + 1 do
		local item = ListUtil.getRandomElement(tMission.dropoffTriggers)
		local trigger = g_missionManager.transportTriggers[item.index]
	...
	local object = ListUtil.getRandomElement(tMission.objects)
...
function TransportMission:calculateReward(multiplier)
	...
	return (driveReward + handleReward) * difficultyMultiplier * multiplier
end
...
function TransportMission:finish(success)
	...
	if success then
		g_currentMission:addIngameNotification(FSBaseMission.INGAME_NOTIFICATION_OK, g_i18n:getText("fieldJob_transportFinished"))
	else
		g_currentMission:addIngameNotification(FSBaseMission.INGAME_NOTIFICATION_CRITICAL, g_i18n:getText("fieldJob_transportFailed"))
	end
end
...
function TransportMission:createHotspots()
	self.pickupHotspot = self:createHotspot({
		0.9301,
		0.6404,
		0.0439,
		1
	}, self:getPickupTrigger())
	self.dropoffHotspot = self:createHotspot({
		0.9301,
		0,
		0.0439,
		1
	}, self:getDropoffTrigger())
end
...
function TransportMission:createHotspot(color, trigger)
	...
	mapHotspot:setBackground(MapHotspot.DEFAULT_FILENAME, MapHotspot.UV.DEFAULT_MISSION, nil, , )
	mapHotspot:setLinkedNode(trigger.triggerId)
	mapHotspot:setIconScale(0.5)
...
function TransportMission:createObject(x, y, z, rx, ry, rz)
	local transportObject = MissionPhysicsObject:new(g_currentMission:getIsServer(), g_currentMission:getIsClient())
