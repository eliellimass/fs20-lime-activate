...
function PlowMission:new(isServer, isClient, customMt)
	...
	self.completionModifier = DensityMapModifier:new(g_currentMission.terrainDetailId, g_currentMission.terrainDetailTypeFirstChannel, g_currentMission.terrainDetailTypeNumChannels)
	self.completionFilter = DensityMapFilter:new(self.completionModifier)

	self.completionFilter:setValueCompareParams("equal", g_currentMission.plowValue)
...
function PlowMission:completeField()
	for i = 1, table.getn(self.field.maxFieldStatusPartitions) do
		g_fieldManager:setFieldPartitionStatus(self.field, self.field.maxFieldStatusPartitions, i, nil, FieldManager.FIELDSTATE_PLOWED, 0, self.sprayFactor, self.fieldSpraySet, g_currentMission.plowCounterMaxValue)
	end
end

function PlowMission.canRunOnField(field, sprayFactor, fieldSpraySet, fieldPlowFactor, limeFactor, maxWeedState)
