...
function SprayMission:new(isServer, isClient, customMt)
	...
	if isServer then
		...
		self.completionModifier = DensityMapModifier:new(ids.id, weedType.startStateChannel, weedType.numStateChannels)
...
function SprayMission:start(...)
	...
	self.completionFilter:setValueCompareParams("equal", self:getNewWeedState())

	return true
end

function SprayMission.canRunOnField(field, sprayFactor, fieldSpraySet, fieldPlowFactor, limeFactor, maxWeedState)
	...
	return maxWeedState >= 2 and maxWeedState <= 3, FieldManager.FIELDSTATE_GROWING, maxGrowthState, maxWeedState
end
