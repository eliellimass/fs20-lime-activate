...
function GamepadSigninScreen:new(inGameMenu, shopMenu, achievementManager, settingsModel)
	local self = {}
	self = ScreenElement:new(nil, GamepadSigninScreen_mt)
...
function GamepadSigninScreen:draw()
...
function GamepadSigninScreen:signIn()
	...
	if GS_PLATFORM_TYPE == GS_PLATFORM_TYPE_XBOXONE then
		local curNickname = g_gameSettings:getValue("nickname")

		if curNickname ~= g_lastUserName then
			resumeGame = false
		end
	end
	...
	if g_tempDeepLinkingInfo ~= nil then
		...
		if GS_PLATFORM_TYPE == GS_PLATFORM_TYPE_XBOXONE and requestUserName ~= "" and g_gameSettings:getValue("nickname") ~= requestUserName then
