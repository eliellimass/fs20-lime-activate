...
function HUDDisplayElement.new(subClass, overlay, parentHudElement)
	if not subClass or subClass == HUDDisplayElement then
		subClass = HUDDisplayElement_mt
	end

	local self = HUDDisplayElement:superClass().new(subClass, overlay, parentHudElement)
	self.origY = 0
	self.origX = 0
