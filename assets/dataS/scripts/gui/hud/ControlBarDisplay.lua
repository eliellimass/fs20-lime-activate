...
function ControlBarDisplay.new(hud, hudAtlasPath)
	...
	local self = ControlBarDisplay:superClass().new(ControlBarDisplay_mt, backgroundOverlay, nil)
	self.hud = hud
	self.uiScale = 1
...
function ControlBarDisplay:removeControlButton(controlButton)
	...
	for _, element in ipairs(controlButton.elements) do
		ListUtil.removeElementFromList(self.hudElements, element)
	end

	ListUtil.removeElementFromList(self.controlButtons, controlButton)

	if controlButton.glyphElement ~= nil then
		ListUtil.removeElementFromList(self.inputGlyphs, controlButton.glyphElement)
	end
...
function ControlBarDisplay:removeFillLevelDisplay(fillLevelDisplay)
	ListUtil.removeElementFromList(self.fillTypeDisplays, fillLevelDisplay)
	self:removeControlButton(fillLevelDisplay)

	if fillLevelDisplay.glyphElement ~= nil then
		ListUtil.removeElementFromList(self.inputGlyphs, fillLevelDisplay.glyphElement)
	end
end
...
function ControlBarDisplay:updatePositionState(force)
	...
	if self.lastInputHelpMode ~= GS_INPUT_HELP_MODE_GAMEPAD and (self.vehicle ~= nil and self.vehicle:getIsAIActive() or self.lastGyroscopeSteeringState) then
		self:setPositionState(ControlBarDisplay.STATE_AI_TOUCH, force)
...
function ControlBarDisplay:setPositionState(state, force)
	if state ~= self.lastPositionState then
		...
		elseif state == ControlBarDisplay.STATE_AI_TOUCH then
			targetX, targetY = unpack(self.aiOffset)
...
function ControlBarDisplay:setSwitchIconsAlpha(alphaValue)
	for i, element in ipairs(self.switchElements) do
