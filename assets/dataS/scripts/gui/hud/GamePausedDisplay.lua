...
function GamePausedDisplay.new(hudAtlasPath)
	...
	local self = GamePausedDisplay:superClass().new(GamePausedDisplay_mt, backgroundOverlay, nil)
	self.pauseText = ""
...
function GamePausedDisplay:createComponents(hudAtlasPath)
	...
	local frameElement = HUDFrameElement:new(hudAtlasPath, baseX, baseY, width, height)
