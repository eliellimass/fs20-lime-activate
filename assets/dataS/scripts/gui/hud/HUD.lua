...
source("dataS/scripts/gui/hud/MapHotspot.lua")
...
source("dataS/scripts/gui/hud/FieldInfoDisplay.lua")
...
function HUD.new(isServer, isClient, isConsoleVersion, messageCenter, l10n, inputManager, inputDisplayManager, modManager, fillTypeManager, fruitTypeManager, guiSoundPlayer, currentMission, farmManager, farmlandManager, customMt)
	...
	self.platformCommunicationElement = nil
...
function HUD:createDisplayComponents(uiScale)
	...
	if g_isPresentationVersion and g_isPresentationVersionLogoEnabled then
		...
		local overlay = Overlay:new("dataS2/menu/presentationVersionLogo.png", g_safeFrameOffsetX, g_safeFrameOffsetY, width, height)
	...
	if self.isConsoleVersion then
		self.platformCommunicationElement = self.speakerDisplay
	else
		self.platformCommunicationElement = self.chatWindow
	end

	self.inputHelp = InputHelpDisplay.new(g_baseHUDFilename, self.messageCenter, self.inputManager, self.inputDisplayManager, self.ingameMap, self.platformCommunicationElement, self.popupMessage, self.isConsoleVersion)
	...
	self.fieldInfoDisplay = FieldInfoDisplay.new(g_baseHUDFilename, self.l10n, self.fruitTypeManager, self.currentMission, self.farmManager, self.farmlandManager)

	self.fieldInfoDisplay:setScale(uiScale)
	self.fieldInfoDisplay:storeOriginalPosition()
	self.fieldInfoDisplay:setVisible(false, false)
	table.insert(self.displayComponents, self.fieldInfoDisplay)
	...
	self.sideNotifications = SideNotification.new(g_baseHUDFilename)
...
function HUD:drawCommunicationDisplay()
	if self.isVisible and not self:getIsFading() then
		...
		self.platformCommunicationElement:draw()
	end
end
...
function HUD:drawInGameMessageAndIcon()
	...
	local iconY = posY - self.inGameIcon.height * 1.05
...
function HUD:showBlinkingWarning(text, duration, priority)
	...
	if self.blinkingWarning == nil or self.blinkingWarning.priority <= priority and text ~= self.blinkingWarning.text then
		self.blinkingWarning = {
			text = text,
			duration = Utils.getNoNil(duration, 2000),
			priority = priority
		}
...
function HUD:showMoneyChange(moneyType, text)
	if self.moneyChanges[moneyType.id] ~= nil and self.moneyChanges[moneyType.id] ~= 0 then
		...
		if change > 0 then
			self:addSideNotification(FSBaseMission.INGAME_NOTIFICATION_OK, string.format("+ %s%s", g_i18n:formatMoney(change, 0, true), text))
			self.guiSoundPlayer:playSample(GuiSoundPlayer.SOUND_SAMPLES.TRANSACTION)
		elseif change <= -1 then
			self:addSideNotification(FSBaseMission.INGAME_NOTIFICATION_CRITICAL, string.format("- %s%s", g_i18n:formatMoney(math.abs(change), 0, true), text))
			self.guiSoundPlayer:playSample(GuiSoundPlayer.SOUND_SAMPLES.TRANSACTION)
		end
...
function HUD:addSideNotification(color, text, duration)
	self.sideNotifications:addNotification(text, color, duration or self.ingameNotificationTime)
end

function HUD:addTopNotification(title, text, info, icon, duration, notification)
	self.topNotification:setNotification(title, text, info, icon, duration)
end
...
function HUD:onMapVisibilityChange(isMapVisible)
	self.speakerDisplay:onMapVisibilityChange(isMapVisible)
end
...
function HUD:setFieldInfoVisible(isVisible)
	self.fieldInfoDisplay:setEnabled(isVisible)
end
...
function HUD:showFillDogBowlContext()
...
function HUD:setPlayer(player)
	...
	self.fieldInfoDisplay:setPlayer(player)
end
...
function HUD:update(dt)
	...
	self.fieldInfoDisplay:update(dt)
	...
	self.platformCommunicationElement:update(dt)
...
function HUD:fadeScreen(direction, duration, callbackFunc, callbackTarget)
	local startAlpha = 0
	local endAlpha = 1
...
function HUD:loadIngameMap(ingameMapFilename, ingameMapWidth, ingameMapHeight)
	self.ingameMap:loadMap(ingameMapFilename, ingameMapWidth, ingameMapHeight)
end

function HUD:setIngameMapSize(sizeIndex)
	self.ingameMap:toggleSize(sizeIndex)
	self.ingameMap:updateFilters()
end
...
function HUD:setInGameIconOnPickup(fillType, amount, text, duration)
	if fillType == FillType.EGG then
		if self.inGameIcon.fileName ~= "dataS2/menu/hud/egg.png" then
			self.inGameIcon:setIcon("dataS2/menu/hud/egg.png")
		end
...
function HUD:scrollChatMessages(delta, numMessages)
	self.chatWindow:scrollChatMessages(delta, numMessages)
end
