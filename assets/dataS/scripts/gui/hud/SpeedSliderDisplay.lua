...
function SpeedSliderDisplay.new(hud, hudAtlasPath)
	...
	local self = SpeedSliderDisplay:superClass().new(SpeedSliderDisplay_mt, backgroundOverlay, nil)
	self.hud = hud
	self.uiScale = 1
...
function SpeedSliderDisplay:setSliderState(state, force)
	if self.sliderState ~= state or force then
		if state then
			self:showSlider()
		else
			self:hideSlider()
...
function SpeedSliderDisplay:hideSlider()
	...
	if self.vehicle == nil and self.player ~= nil and self.lastInputHelpMode == GS_INPUT_HELP_MODE_GAMEPAD then
		yOffset = yOffset * 2
	end

	local startX, startY = self:getPosition()

	if startY ~= self.origY - yOffset then
...
function SpeedSliderDisplay:onInputHelpModeChange(inputHelpMode)
	...
	self:updateVisibilityState(true)
end
...
function SpeedSliderDisplay:updateVisibilityState(force)
	local sliderState = self:getIsSliderActive()

	if sliderState ~= self.sliderState or force then
		self:setSliderState(sliderState, force)
	end
end
