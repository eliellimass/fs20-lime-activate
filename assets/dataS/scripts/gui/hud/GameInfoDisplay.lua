...
function GameInfoDisplay.new(hudAtlasPath, moneyUnit, l10n)
	...
	local self = GameInfoDisplay:superClass().new(GameInfoDisplay_mt, backgroundOverlay, nil)
...
function GameInfoDisplay:setMoneyUnit(moneyUnit)
	...
	self.moneyIconOverlay:setUVs(GuiUtils.getUVs(GameInfoDisplay.UV.MONEY_ICON[moneyUnit]))
end
...
function GameInfoDisplay:getWeatherStates()
	...
	local dayPlus6h, timePlus6h = env:getDayAndDayTime(env.dayTime + sixHours, env.currentDay)
	local weatherState = env.weather:getWeatherTypeAtTime(env.currentDay, env.dayTime)
	local nextWeatherState = env.weather:getWeatherTypeAtTime(dayPlus6h, timePlus6h)
...
function GameInfoDisplay:updateWeather(dt)
	...
	local weatherChanging = weatherState ~= nextWeatherState
...
function GameInfoDisplay:updateSizeAndPositions()
	...
	self:setDimension(width, self:getHeight())
...
function GameInfoDisplay:storeScaledValues()
	...
	local rightX = baseX + width
	local bottomY = baseY - height
	...
	self.moneyTextPositionY = moneyBoxPosY + self.moneyBox:getHeight() * 0.5 + textOffY
	local timeBoxPosX, timeBoxPosY = self.timeBox:getPosition()
	local timeBoxWidth = self.timeBox:getWidth()
	...
	self.timeTextPositionX = timeBoxPosX + timeBoxWidth + textOffX
	self.timeTextPositionY = timeBoxPosY + textOffY
	...
	self.timeScaleTextPositionX = timeBoxPosX + timeBoxWidth + textOffX
	self.timeScaleTextPositionY = timeBoxPosY + textOffY
...
function GameInfoDisplay.createBackground()
	...
	return Overlay:new(nil, posX - width, posY - height, width, height)
end

function GameInfoDisplay:createComponents(hudAtlasPath)
	local topRightX, topRightY = GameInfoDisplay.getBackgroundPosition(1)
	local bottomY = topRightY - self:getHeight()
	local marginWidth, marginHeight = self:scalePixelToScreenVector(GameInfoDisplay.SIZE.BOX_MARGIN)
	...
	sepX = rightX
	rightX = self:createTutorialBox(hudAtlasPath, rightX - marginWidth, bottomY) - marginWidth
	separator = self:createVerticalSeparator(hudAtlasPath, sepX, centerY)
...
function GameInfoDisplay:createMoneyBox(hudAtlasPath, rightX, bottomY)
	...
	iconOverlay:setUVs(GuiUtils.getUVs(GameInfoDisplay.UV.MONEY_ICON[self.moneyUnit]))
...
function GameInfoDisplay:createTimeBox(hudAtlasPath, rightX, bottomY)
	...
	posY = bottomY + arrowOffY
	posX = rightX + arrowOffX
