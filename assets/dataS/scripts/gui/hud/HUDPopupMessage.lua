...
function HUDPopupMessage:new(hudAtlasPath, l10n, inputManager, inputDisplayManager, ingameMap, guiSoundPlayer)
	...
	local self = HUDPopupMessage:superClass().new(HUDPopupMessage_mt, backgroundOverlay, nil)
	self.l10n = l10n
...
function HUDPopupMessage:showMessage(title, message, duration, controls, callback, target)
	if duration == 0 then
		duration = HUDPopupMessage.MIN_DURATION + string.len(message) * HUDPopupMessage.DURATION_PER_CHARACTER
...
function HUDPopupMessage:startMessage()
	...
	self.ingameMap:toggleSize(IngameMap.STATE_MEDIUM, true)
...
function HUDPopupMessage:draw()
	if not self.isMenuVisible and self:getVisible() and self.currentMessage ~= nil then
		...
		for i = 1, #self.currentMessage.controls do
			...
			local offX = self.inputRowTextX
			local offY = self.inputRowTextY
...
function HUDPopupMessage:createComponents(hudAtlasPath)
	...
	local frame = HUDFrameElement:new(hudAtlasPath, basePosX, basePosY, baseWidth, baseHeight)
...
function HUDPopupMessage:createInputRow(hudAtlasPath, posX, posY)
	...
	local offX, offY = self:scalePixelToScreenVector(HUDPopupMessage.POSITION.SEPARATOR)
