...
function InputHelpDisplay.new(hudAtlasPath, messageCenter, inputManager, inputDisplayManager, ingameMap, communicationDisplay, ingameMessage, isConsoleVersion)
	...
	local self = InputHelpDisplay:superClass().new(InputHelpDisplay_mt, backgroundOverlay, nil)
	...
	self.vehicle = nil
	self.vehicleTypesBuffer = {}
	self.vehicleSpecializationsBuffer = {}
...
function InputHelpDisplay:setVehicle(vehicle)
	if self.vehicle and vehicle ~= self.vehicle then
...
local function getVehicleTypeHash(vehicles)
	...
	for vehicle, vehicleType in pairs(vehicles) do
...
function InputHelpDisplay:updateSizeAndPositions()
	local totalSize = -self.frameOffsetY
...
function InputHelpDisplay:refreshHUDExtensions()
	...
	for vehicle, specs in pairs(self.vehicleSpecializationsBuffer) do
		for _, spec in pairs(specs) do
			...
			if hudExtension == nil and VehicleHUDExtension.hasHUDExtensionForSpecialization(spec) then
				...
				self.vehicleHudExtensions[spec] = hudExtension
			end
		end
	end
end
...
function InputHelpDisplay:updateHUDExtensions()
	if self.vehicle ~= nil then
		local prevHash = getVehicleTypeHash(self.vehicleTypesBuffer)

		clearTable(self.vehicleTypesBuffer)
		clearTable(self.vehicleSpecializationsBuffer)
		self:collectVehicleTypesAndSpecializations(self.vehicleTypesBuffer, self.vehicleSpecializationsBuffer, 1, self.vehicle, self.vehicle)

		local currentHash = getVehicleTypeHash(self.vehicleTypesBuffer)

		if prevHash ~= currentHash then
			self.requireHudExtensionsRefresh = true
		end
	end
...
function InputHelpDisplay:collectVehicleTypesAndSpecializations(vehicleTypes, specs, depth, vehicle, rootVehicle)
	if vehicle.getAttachedImplements == nil then
		return
	end

	vehicleTypes[vehicle] = vehicle.typeName
	local vehicleSpecs = specs[vehicle]

	if not vehicleSpecs then
		vehicleSpecs = {}
		specs[vehicle] = vehicleSpecs
	end

	for _, v in pairs(vehicle.specializations) do
		table.insert(vehicleSpecs, v)
	end

	local attachedImplements = vehicle:getAttachedImplements()

	for _, implement in pairs(attachedImplements) do
		local attached = implement.object

		if attached ~= nil and attached.specializations ~= nil and depth <= 3 then
			self:collectVehicleTypesAndSpecializations(vehicleTypes, specs, depth + 1, attached, rootVehicle)
		end
	end
end
...
function InputHelpDisplay:updateEntries(helpElements)
	...
	if self.extensionsHeight > 0 and not self.ingameMap:getIsFullSize() then
	...
	for i = 1, entryCount do
		...
		if i <= #helpElements then
			...
			entry:setVisible(#helpElements[i].buttons > 0 or #helpElements[i].keys > 0)
...
function InputHelpDisplay:updateComboHeaders(useGamepadButtons, pressedComboMaskMouse, pressedComboMaskGamepad)
	...
	self.hasComboCommands = next(comboActionStatus) ~= nil
	...
	if self.extensionsHeight > 0 then
		...
		for _, extension in pairs(self.vehicleHudExtensions) do
			...
			if extension:canDraw() and usedHeight + extHeight <= self.extensionsHeight then
				posY = extension:draw(leftPosX + self.extraTextOffsetX, leftPosX + width + self.helpTextOffsetX, posY)
...
function InputHelpDisplay:getMaxEntryCount(prio)
...
function InputHelpDisplay:createComponents(hudAtlasPath)
	...
	local maxEntries = self:getMaxEntryCount()
...
function InputHelpDisplay:createComboHeader(hudAtlasPath, frameX, frameY, combos, boxSize, separatorPositions)
	...
	for i, combo in ipairs(combos) do
		...
		local glyphWidth = glyphElement:getGlyphWidth()
		local glyphPosX = frameX + boxWidth * count + (boxWidth - glyphWidth) * 0.5
		...
		elseif i == #combos then
			local offX = self:scalePixelToScreenWidth(InputHelpDisplay.POSITION.AXIS_ICON[1]) - glyphWidth
		...
		if count > 0 and count <= #separatorPositions then
			local sepX, sepY = self:scalePixelToScreenVector(separatorPositions[count])
			sepY = sepY + boxHeight * 0.5
			local separator = self:createVerticalSeparator(hudAtlasPath, frameX + sepX, posY + sepY)

			headerElement:addChild(separator)
		end

		count = count + 1
	end

	local separator = self:createHorizontalSeparator(hudAtlasPath, frameX, posY)
...
function InputHelpDisplay:createEntries(hudAtlasPath, frameX, frameY, count)
	...
	for i = 1, count do
		...
		local entry = self:createEntry(hudAtlasPath, posX, posY, entryWidth, entryHeight)

		table.insert(self.entries, entry)
		table.insert(self.entryGlyphWidths, 0)
		entriesFrame:addChild(entry)

		local sep = self:createHorizontalSeparator(hudAtlasPath, posX, posY)

		table.insert(self.horizontalSeparators, sep)
		entriesFrame:addChild(sep)
	end
end

InputHelpDisplay.WIDTH = 465
InputHelpDisplay.HEADER_HEIGHT = 72
InputHelpDisplay.MIN_MAP_SPACING = IngameMap.SIZE.INPUT_ICON[2] * 1.1
