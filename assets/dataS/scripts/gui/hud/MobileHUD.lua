...
function MobileHUD:createDisplayComponents(uiScale)
	...
	if g_isPresentationVersion and g_isPresentationVersionLogoEnabled then
		...
		local overlay = Overlay:new("dataS2/menu/presentationVersionLogo.png", g_safeFrameOffsetX + xOffset, 1 - g_safeFrameOffsetY / 2 - height + yOffset, width, height)
	...
	local _, yOffset = getNormalizedScreenValues(0, -150)
	self.poiInfoDisplay = HUDTextDisplay:new(0.5, 1 + yOffset, HUD.TEXT_SIZE.VEHICLE_NAME, RenderText.ALIGN_CENTER, HUD.COLOR.VEHICLE_NAME, true)

	self.poiInfoDisplay:setTextShadow(true, HUD.COLOR.VEHICLE_NAME_SHADOW)
end
...
function MobileHUD:drawPOIInfo(text)
