...
function HUDSliderElement.new(customMt, overlay, backgroundOverlay, touchAreaOffsetX, touchAreaOffsetY, touchAreaPressedGain, transAxis, minTrans, centerTrans, maxTrans, lockTrans)
	local mt = HUDSliderElement_mt

	if customMt ~= nil and customMt ~= HUDSliderElement then
		mt = customMt
	end

	local self = HUDSliderElement:superClass().new(mt, overlay)
	self.position = {
		overlay.x,
		overlay.y
	}
