		...
		for i = 0, numCategories - 1 do
			if i ~= ModHubController.CATEGORY_ID_CONTEST or not GS_IS_CONSOLE_VERSION and getNumOfMods(i) > 0 then
				...
				if localCategory ~= nil then
					...
					g_logManager:warning("Could not find modhub category %s in modHub.xml", categoryName)
				end
			end
		end
...
function ModHubController:getModsByCategory(categoryId)
	local mods = {}
...
function ModHubController:uninstall(modId)
	...
	if success then
		...
		local mod = g_modManager:getModByFileHash(hash)
		...
		for i = 0, numOfMods - 1 do
			if list[modId] == nil then
				...
				if sStart ~= nil then
					...
					local sStart, sEnd = author:lower():find(text)
...
function ModHubController:updateRecommendationSystem()
	...
	for _, done in pairs(self.gameSettings.tutorialsDone) do
		if done then
			numTutorialsPlayed = numTutorialsPlayed + 1
		end
	end

	local isLastUsedCharacterMale = self.gameSettings:getValue(GameSettings.SETTING.PLAYER_MODEL_INDEX) == 1
