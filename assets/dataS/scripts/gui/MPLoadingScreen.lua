MPLoadingScreen = {
	...
	PERCENTAGE_PER_MS = 1.5833333333333333e-05,
	LOAD_TARGETS = {
		ADDITIONAL_FILES = 0.6,
		TERRAIN = 0.65,
		ITEMS = 0.95,
...
function MPLoadingScreen:new(target, custom_mt, missionCollaborators, savegameController, loadFunction)
	...
	self.defaultMapPreviewFilename = MPLoadingScreen.DEFAULT_MAP_PREVIEW_FILENAME
	self.loadTargets = {}

	for name, target in pairs(MPLoadingScreen.LOAD_TARGETS) do
		table.insert(self.loadTargets, target)
	end
...
function MPLoadingScreen:onGuiSetupFinished()
	...
	self.defaultMapPreviewFilename = self.mapSelectionPreview.overlay.filename
end
...
function MPLoadingScreen:onClose()
	...
	self.mapSelectionPreview:setImageFilename(self.defaultMapPreviewFilename)
	leaveCpuBoostMode()
end
...
function MPLoadingScreen:update(dt)
	...
	if (GS_IS_CONSOLE_VERSION or GS_PLATFORM_TYPE == GS_PLATFORM_TYPE_SWITCH) and MPLoadingScreen.STATE_WAIT_FOR_ACCEPT < self.state and self.state < MPLoadingScreen.STATE_READY then
		...
		self.loadingBar:setSize(self.loadingBar.parent.size[1] * self.loadPercentage)
		self.loadingBarPercentage:setText(string.format("%d%%", self.loadPercentage * 100))
	else
		self.loadingBarBg:setVisible(false)
	end

	if self.state == MPLoadingScreen.STATE_WAIT_FOR_ACCEPT and GS_PLATFORM_TYPE == GS_PLATFORM_TYPE_PS4 and self.acceptCancelTimer > 0 then
...
function MPLoadingScreen:startClient()
	...
	g_deferredLoadingManager:addTask(function ()
		g_client = Client:new()
		self.missionCollaborators.server = g_server
		self.missionCollaborators.client = g_client
...
function MPLoadingScreen:onSavegameLoaded(errorCode, savegameDirectory)
	...
	if errorCode == Savegame.ERROR_OK then
		...
		if savegame.bansXML == nil or not fileExists(savegame.bansXML) then
			savegame.bansXMLLoad = nil
		else
			savegame.bansXMLLoad = savegame.bansXML
		end
...
function MPLoadingScreen:onReadyToStart()
	if g_currentMission:canStartMission() then
		...
		if g_dedicatedServerInfo ~= nil or GS_IS_MOBILE_VERSION then
			self:onClickOk()
		end
	else
...
function MPLoadingScreen:initializeLoading()
	...
	g_deferredLoadingManager:addTask(function ()
		g_vehicleTypeManager:validateVehicleTypes()
		self:hitLoadingTarget(MPLoadingScreen.LOAD_TARGETS.VEHICLE_VALIDATION)
	end)
	g_deferredLoadingManager:addTask(function ()
		g_vehicleTypeManager:finalizeVehicleTypes()
	end)
	g_deferredLoadingManager:addTask(function ()
		Vehicle.init()
	...
	g_deferredLoadingManager:addTask(function ()
		g_placeableTypeManager:initPlaceableTypes()
	end)
	setCamera(0)
...
function MPLoadingScreen:onServerInfoDetails(id, ip, port, name, language, capacity, numPlayers, mapName, mapId, hasPassword, isLanServer, modTitles, modHashs)
	if id == self.missionDynamicInfo.serverId then
		...
		for i = 1, table.getn(modHashs) do
			local modItem = g_modManager:getModByFileHash(modHashs[i])

			if modItem == nil then
				local parts = StringUtil.splitString(";", modTitles[i])
		...
		g_logManager:warning("Invalid server id '%s' for server '%s'. Requested server id '%s'!", tostring(id), tostring(name), tostring(self.missionDynamicInfo.serverId))
	end
end
...
function MPLoadingScreen:onMasterServerConnectionReady()
	...
	for i = 1, table.getn(self.missionDynamicInfo.mods) do
		...
		local modTitleStr = ServerDetailScreen.packModInfo(modItem.title, modItem.version, modItem.author)
	...
	masterServerAddServer(self.missionDynamicInfo.serverPort, self.missionDynamicInfo.serverName, self.missionDynamicInfo.password, self.missionDynamicInfo.capacity, 0, map.title, self.missionInfo.mapId, self.missionDynamicInfo.allowOnlyFriends, getIsBadUserReputation(), g_createGameScreen.usePendingInvites)
	self:initializeLoading()
end
