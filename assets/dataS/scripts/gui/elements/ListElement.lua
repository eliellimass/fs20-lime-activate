...
function ListElement:raiseSliderUpdateEvent()
	if self.sliderElement ~= nil then
		self.sliderElement:onListUpdate(self)
	end
end
...
function ListElement:calculateFirstVisibleItem(index)
	...
	elseif self:getItemCount() == 0 then
		newFirstVisibleItem = 0
	end

	return MathUtil.clamp(newFirstVisibleItem, 1, self:getItemCount())
end
...
function ListElement:setSelectedIndex(index, force, direction)
	...
	if self.listItems[newIndex] ~= nil and not self.listItems[newIndex].allowFocus then
...
function ListElement:updateAlternatingBackground(forceProfile)
	...
	for k, item in ipairs(self.listItems) do
...
function ListElement:getRowColumnForScreenPosition(posX, posY)
	local topPos = self.absPosition[2] + self.absSize[2] - self.listItemStartYOffset - self.listItemHeight
	local leftPos = self.absPosition[1] + self.listItemStartXOffset
...
function ListElement:onFocusActivate()
	local selectedElement = self.listItems[self:getSelectedElementIndex()]
...
function ListElement:onFocusEnter()
	if self.selectedIndex > 0 and #self.listItems > 0 then
		self.listItems[self:getSelectedElementIndex()]:setSelected(true)
	end
end
...
function ListElement:applyElementSelection()
	local selectedElementIndex = self:getSelectedElementIndex()

	if self.firstVisibleItem > 0 then
		for i, element in ipairs(self.elements) do
			if element.setSelected ~= nil then
				element:setSelected(selectedElementIndex == i)
			end
		end
	end
end
...
function ListElement:verifyListItemConfiguration()
	if not self.listItemAutoSize then
		if self.listItemWidth == 0 then
			g_logManager:devError("Missing required listItemWidth configuration for list with [id] %s and profile [%s]", self.id, self.profile)
		end

		if self.listItemHeight == 0 then
			g_logManager:devError("Missing required listItemHeight configuration for list with [id] %s and profile [%s]", self.id, self.profile)
		end
	end
end
