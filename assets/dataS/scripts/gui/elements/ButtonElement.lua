...
function ButtonElement:loadFromXML(xmlFile, key)
	...
	if inputActionName ~= nil and InputAction[inputActionName] ~= nil then
		...
		GuiOverlay.loadOverlay(self, self.icon, "icon", self.imageSize, nil, xmlFile, key)
...
function ButtonElement:loadProfile(profile, applyProfile)
	...
	if inputActionName ~= nil and InputAction[inputActionName] ~= nil then
		...
		GuiOverlay.loadOverlay(self, self.icon, "icon", self.imageSize, profile, nil, )
		GuiOverlay.createOverlay(self.icon)
	end
...
function ButtonElement:copyAttributes(src)
	...
	if src.isTouchButton and GS_IS_MOBILE_VERSION then
		...
		self.touchIconSize = ListUtil.copyTable(src.touchIconSize)
	end

	self.iconSize = ListUtil.copyTable(src.iconSize)
	self.iconTextOffset = ListUtil.copyTable(src.iconTextOffset)
	self.hotspot = ListUtil.copyTable(src.hotspot)
...
function ButtonElement:onOpen()
	...
	if self.inputActionName ~= nil and not self.hasLoadedInputGlyph then
		self:loadInputGlyph()
	end
end
...
function ButtonElement:keyEvent(unicode, sym, modifier, isDown, eventUsed)
	if self:getIsActive() and ButtonElement:superClass().keyEvent(self, unicode, sym, modifier, isDown, eventUsed) then
		return true
	end
...
function ButtonElement:draw()
	...
	GuiOverlay.renderOverlay(self.overlay, self.absPosition[1], self.absPosition[2], self.size[1], self.size[2], self:getOverlayState())
	...
	if self.keyDisplayText ~= nil and self.isKeyboardMode then
		if not self.hideKeyboardGlyph then
			...
			self.keyOverlay:renderButton(self.keyDisplayText, iconXPos, iconYPos, iconSizeY, self.textAlignment, overlayState == GuiOverlay.STATE_DISABLED)
		end
	elseif self.isTouchMode then
		...
		if self.touchIcon ~= nil then
			...
			GuiOverlay.renderOverlay(self.touchIcon, iconXPos, touchIconYPos, self.touchIconSize[1], self.touchIconSize[2], overlayState)
		end
	else
		GuiOverlay.renderOverlay(self.icon, iconXPos, iconYPos, iconSizeX, iconSizeY, overlayState)
	end
...
function ButtonElement:updateSize()
	if not self.fitToContent then
		return
	end
