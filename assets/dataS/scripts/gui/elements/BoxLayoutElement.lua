...
function BoxLayoutElement:delete()
	BoxLayoutElement:superClass().delete(self)
end
...
function BoxLayoutElement:getIsElementIncluded(element, ignoreVisibility)
	return not element.ignoreLayout and ignoreVisibility or element:getIsVisible() and self.useFullVisibility or element.visible and not self.useFullVisibility
end
...
function BoxLayoutElement:focusLinkCells(flowCells)
	...
	for _, flow in pairs(flowCells) do
		for _, cell in pairs(flow) do
			self:focusLinkChildElement(cell.element, prevElement, firstElement, lastElement)

			prevElement = cell.element
		end
	end
end
