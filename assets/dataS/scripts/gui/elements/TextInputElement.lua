...
TextInputElement.INITIAL_REPEAT_DELAY = 500
TextInputElement.MIN_REPEAT_DELAY = 100
...
function TextInputElement:copyAttributes(src)
	...
	self.cursorOffset = ListUtil.copyTable(src.cursorOffset)
	self.cursorSize = ListUtil.copyTable(src.cursorSize)
...
function TextInputElement:inputEvent(action, value, eventUsed)
	...
	if self.imeQueued then
		if not self.imeActive then
			if not action == InputAction.MENU_ACCEPT then
				if imeOpen(self.text, self.imeTitle, self.imeDescription, self.imePlaceholder, self.imeKeyboardType, Utils.getNoNil(self.maxCharacters, 512)) then
					self.imeActive = true
				elseif not self.imeActive then
					self:raiseCallback("onEnterPressedCallback", self)
...
function TextInputElement:draw()
	...
	TextInputElement:superClass().draw(self)
	...
	if self.areBackDotsVisible then
		local additionalDisplacement = self:drawTextPart(self.backDotsText, textXPos, displacementX, textYPos)
		displacementX = displacementX + additionalDisplacement
	end
...
function TextInputElement:drawCursor(textXPos, displacementX, textYPos)
	if self.cursorBlinkTime < self.cursorBlinkInterval then
		GuiOverlay.renderOverlay(self.cursor, textXPos + displacementX + self.cursorOffset[1], textYPos + self.cursorOffset[2], self.cursorSize[1], self.cursorSize[2])
	end
