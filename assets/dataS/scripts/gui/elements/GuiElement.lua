...
function GuiElement:loadProfile(profile, applyProfile)
	...
	if imageSize ~= nil then
		local x, y = StringUtil.getVectorFromString(imageSize)
	...
	if outputSize ~= nil then
		local x, y = StringUtil.getVectorFromString(outputSize)
...
function GuiElement:copyAttributes(src)
	...
	self.size = ListUtil.copyTable(src.size)
	self.margin = ListUtil.copyTable(src.margin)
	...
	self.outputSize = ListUtil.copyTable(src.outputSize)
	self.imageSize = ListUtil.copyTable(src.imageSize)
	...
	self.frameColors = ListUtil.copyTableRecursively(src.frameColors)
	self.frameOverlayVisible = ListUtil.copyTable(src.frameOverlayVisible)
	self.focusId = src.focusId
	self.focusChangeData = ListUtil.copyTable(src.focusChangeData)
	self.focusActive = src.focusActive
	self.isAlwaysFocusedOnOpen = src.isAlwaysFocusedOnOpen
	self.position = ListUtil.copyTable(src.position)
	self.absPosition = ListUtil.copyTable(src.absPosition)
	self.absSize = ListUtil.copyTable(src.absSize)
	self.anchors = ListUtil.copyTable(src.anchors)
	self.pivot = ListUtil.copyTable(src.pivot)
end
...
function GuiElement:onGuiSetupFinished()
	...
	if self.toolTipElementId ~= nil then
		...
		if toolTipElement ~= nil then
			...
			g_logManager:warning("toolTipElementId '%s' not found for '%s'!", self.toolTipElementId, self.target.name)
		end
	end
end
	...
	if self.hasFrame then
		for i = 1, 4 do
			if self.frameOverlayVisible[i] then
				...
				drawFilledRect(frame.x, frame.y, frame.width, frame.height, color[1], color[2], color[3], color[4])
			end
		end
	end
end
...
function GuiElement:setPosition(x, y)
	x = Utils.getNoNil(x, self.position[1])
	y = Utils.getNoNil(y, self.position[2])
	self.position = {
		x,
		y
	}
...
function GuiElement:setAbsolutePosition(x, y)
	...
	for _, childElem in ipairs(self.elements) do
		childElem:setAbsolutePosition(childElem.absPosition[1] + xDif, childElem.absPosition[2] + yDif)
	end
...
function GuiElement:getDescendantById(id)
	...
	if id then
		local function findId(element)
			return element.id and element.id == id
		end
...
function GuiElement:getDescendantByName(name)
	...
	if name then
		local function findId(element)
			return element.name and element.name == name
		end
...
GuiElement_mt.__tostring = GuiElement.toString
