...
function FrameElement:new(target, custom_mt)
	local custom_mt = custom_mt or FrameElement_mt
...
function FrameElement:copyAttributes(src)
	...
	for k, v in pairs(src.controlIDs) do
...
function FrameElement:registerControls(controlIDs)
	for _, id in pairs(controlIDs) do
		if self.controlIDs[id] then
			g_logManager:warning("Registered multiple control elements with the same ID '%s'. Check screen setup.", tostring(id))
		else
	...
	if self.debugEnabled or g_uiDebugEnabled then
		for id, isResolved in pairs(self.controlIDs) do
			if not isResolved then
				g_logManager:warning("FrameElement for GUI view '%s' could not resolve registered control element ID '%s'. Check configuration.", tostring(viewName), tostring(id))
			end
		end
	end
end
