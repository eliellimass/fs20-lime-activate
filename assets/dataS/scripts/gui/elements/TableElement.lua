TableElement = {
	...
	DataRow = setmetatable({
		new = function (class, id, columnNames)
	...
	TableRow = setmetatable({
		new = function (class, dataRowIndex, rowElement)
			local self = setmetatable({}, class)
			self.dataRowIndex = dataRowIndex
...
function TableElement:loadFromXML(xmlFile, key)
	...
	for i, name in ipairs(StringUtil.splitString(" ", colNames)) do
...
function TableElement:buildTableRows()
	...
	rowTemplate.parent:removeElement(rowTemplate)
end
...
function TableElement:applyAlternatingBackgroundsToRows()
	...
	local function isRowBackground(element)
		return element.profile and element.profile == self.rowBackgroundProfile
	end
...
function TableElement:updateRows()
	for i, tableRow in ipairs(self.tableRows) do
		...
		if dataIndex <= #self.dataView then
...
function TableElement:inputEvent(action, value, eventUsed)
	local eventUsed = TableElement:superClass().inputEvent(action, value, eventUsed)
