...
function InGameMenuAnimalsFrameMobile:new(subclass_mt, messageCenter, l10n, animalManager, animalFoodManager, fillTypeManager)
	local self = InGameMenuAnimalsFrame:new(subclass_mt or InGameMenuAnimalsFrameMobile_mt, messageCenter, l10n, animalManager, animalFoodManager, fillTypeManager)
...
function InGameMenuAnimalsFrameMobile:updateMenuButtons()
	...
	if self.mode == InGameMenuAnimalsFrameMobile.MODE.LIST then
		...
		if self.selectedHorse ~= nil then
			if not self.selectedHorse:getIsInUse() then
				table.insert(self.menuButtonInfo, self.rideButtonInfo)
			end

			if self.selectedHorse:getDirtScale() > 0.01 then
				table.insert(self.menuButtonInfo, self.cleanButtonInfo)
			end
		end
	end
