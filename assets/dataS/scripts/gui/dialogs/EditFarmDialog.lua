...
EditFarmDialog.COLOR_ELEMENT_NAME = "colorImage"
...
function EditFarmDialog:onClose()
	...
	self:setButtonAction(InputAction.MENU_ACTIVATE)
end
...
function EditFarmDialog:setExistingFarm(farmId)
	...
	self.farmColorPreview:setImageColor(nil, unpack(Farm.COLORS[self.selectedColorIndex]))
...
function EditFarmDialog:onClickActivate()
	...
	if farmName ~= "" then
		if farmName ~= filteredName then
			...
			g_logManager:info("Entered farm name contains profanity and has been adjusted.")
		else
