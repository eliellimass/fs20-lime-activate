...
function SettingsConsoleFrame:onFrameOpen()
	...
	local focusedElement = self.resolutionElement

	if not self.settingsModel:getConsoleIsResolutionVisible() then
		focusedElement = self.renderQualityElement

		if not self.settingsModel:getConsoleIsRenderQualityVisible() then
			focusedElement = self.fovyElement
		end
	end

	FocusManager:setFocus(focusedElement)
