...
function InGameMenuStatisticsFrame:new(subclass_mt)
	local subclass_mt = subclass_mt or InGameMenuStatisticsFrame_mt
...
function InGameMenuStatisticsFrame:onOpen(element)
	InGameMenuStatisticsFrame:superClass().onOpen(self)
...
function InGameMenuStatisticsFrame:onFrameOpen(element)
	...
	for i, header in ipairs(self.tableHeaderBox) do
...
function InGameMenuStatisticsFrame:updateStatistics()
	...
	local numStats = 0
	local statData = self.playerFarm.stats:getStatisticData()
