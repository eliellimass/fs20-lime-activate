...
function InGameMenuContractsFrame:new(subclass_mt, messageCenter, i18n, missionManager)
	local self = TabbedMenuFrameElement:new(nil, subclass_mt or InGameMenuContractsFrame_mt)
	...
	self.contracts = {}
	self.contractsDataSource = GuiDataSource:new()

	self.contractsDataSource:addChangeListener(self, self.onContractsChanged)
...
function InGameMenuContractsFrame:onGuiSetupFinished()
	...
	local function assignDataFunction(guiElement, contract)
		self:assignContractToListItem(guiElement, contract)
	end

	self.contractsList:setDataSource(self.contractsDataSource)
	self.contractsList:setAssignItemDataFunction(assignDataFunction)
end
...
function InGameMenuContractsFrame:getMainElementSize()
	return self.contractsContainer.size
end

function InGameMenuContractsFrame:getMainElementPosition()
	return self.contractsContainer.absPosition
end
...
function InGameMenuContractsFrame:updateButtonsForPermissions()
	self:updateDetailContents(self.contractsList.selectedIndex)
end
...
function InGameMenuContractsFrame:updateList()
	local rememberPosition = self.contractsList:getItemCount() > 0
	local list = g_missionManager:getMissionsList(g_currentMission:getFarmId())
	local hasMissions = table.getn(list) ~= 0
	...
	local function hash(a)
		return 1099511627776.0 * (a.active and 1 or 0) + 549755813888.0 * (a.finished and 1 or 0) + a.mission.generationTime
	end
	...
	for i, mission in ipairs(list) do
	...
	self:sortList()
	self.contractsDataSource:setData(self.contracts)

	local indexToSelect = 1

	if rememberPosition and #self.contracts > 0 then
		for i, contract in ipairs(self.contracts) do
			local mission = contract.mission

			if rememberPosition and self.storedSelected == mission.generationTime then
				indexToSelect = i
			end
		end
	end

	self.contractsList:setSelectedIndex(indexToSelect)
	self:onListSelectionChanged(indexToSelect)
end

function InGameMenuContractsFrame:assignContractToListItem(listItem, contract)
	local mission = contract.mission
	local missionInfo = mission:getData()
	local contractText = listItem:getDescendantByName(InGameMenuContractsFrame.LIST_ITEM_CONTRACT_NAME)

	contractText:setText(missionInfo.jobType)

	local rewardText = listItem:getDescendantByName(InGameMenuContractsFrame.LIST_ITEM_REWARD_NAME)

	rewardText:setText(g_i18n:formatMoney(mission.reward, 0, true, true))

	local fieldText = listItem:getDescendantByName(InGameMenuContractsFrame.LIST_ITEM_FIELD_NAME)

	fieldText:setText(missionInfo.location)

	local indicatorActive = listItem:getDescendantByName(InGameMenuContractsFrame.LIST_ITEM_INDICATOR_ACTIVE_NAME)

	indicatorActive:setVisible(contract.active)

	local indicatorFinished = listItem:getDescendantByName(InGameMenuContractsFrame.LIST_ITEM_INDICATOR_FINISHED_NAME)

	indicatorFinished:setVisible(contract.finished and contract.mission.success)

	local indicatorFinished = listItem:getDescendantByName(InGameMenuContractsFrame.LIST_ITEM_INDICATOR_FAILED_NAME)

	indicatorFinished:setVisible(contract.finished and not contract.mission.success)
end
...
function InGameMenuContractsFrame:updateDetailContents(index)
	local contract = self.contracts[index]
			...
			if hasLeasing then
				...
				for i, v in ipairs(vehicles) do
					...
					if storeItem == nil then
						print("ERROR: Mission uses non-existing vehicle at '" .. v.filename .. "'")
...
function InGameMenuContractsFrame:updateFieldContractInfo(mission)
	...
	self.rewardText:setText(g_i18n:formatMoney(mission.reward, 0, true, true))
...
function InGameMenuContractsFrame:updateFarmersBox(field, npc)
	...
	if npc ~= nil then
		...
		if field ~= nil then
			self.npcFieldBox:setVisible(true)
			self.ownerOfText:setText(string.format(self.i18n:getText("fieldJob_number"), field.fieldId))
			self.areaText:setText(self.i18n:formatArea(field.fieldArea, 2))
		else
			self.npcFieldBox:setVisible(false)
		end
	end
end

function InGameMenuContractsFrame:getSelectedContract()
	local index = self.contractsList:getSelectedDataIndex()
...
function InGameMenuContractsFrame:startContract(leaseVehicles)
	...
	if g_missionManager:hasFarmActiveMission(farmId) then
...
function InGameMenuContractsFrame:onListSelectionChanged(index)
	if index > 0 and index <= table.getn(self.contracts) then
		self:updateDetailContents(index)
