...
InGameMenu.CONTROLS = {
	...
	PAGE_SETTINGS_MOBILE = "pageSettingsMobile",
...
function InGameMenu:new(target, customMt, messageCenter, l10n, inputManager, savegameController, fruitTypeManager, fillTypeManager, isConsoleVersion)
	local self = TabbedMenu:new(target, customMt or InGameMenu_mt, messageCenter, l10n, inputManager)
...
function InGameMenu:setSellingStations(sellingStations)
	self.pagePrices:setSellingStations(sellingStations)
end

function InGameMenu:setAccessibleVehicles(vehicles)
	self.pageGarageOverview:setAccessibleVehicles(vehicles)
end

function InGameMenu:setBanStorage(banStorage)
	if self.pageMultiplayerUsers ~= nil then
		self.pageMultiplayerUsers:setBanStorage(banStorage)
	end
end
...
function InGameMenu:setupMenuPages()
	...
	local orderedDefaultPages = {
		{
			self.pageTutorial,
			self:makeIsTutorialEnabledPredicate(),
			InGameMenu.TAB_UV.TUTORIAL
		},
		{
			self.pageMain,
			mobileFunc,
...
function InGameMenu:setupMenuButtonInfo()
	...
	self.tutorialMenuButtonInfo = {
		...
		{
			inputAction = InputAction.MENU_ACTIVATE,
			text = self.l10n:getText(InGameMenu.L10N_SYMBOL.BUTTON_RESTART),
			callback = onButtonSaveGameFunction
		},
		{
			inputAction = InputAction.MENU_CANCEL,
...
function InGameMenu:setMissionInfo(missionInfo, missionDynamicInfo, missionBaseDirectory, isTutorialMission)
	self.missionInfo = missionInfo
	self.missionDynamicInfo = missionDynamicInfo
	self.isTutorialMission = isTutorialMission

	if self.pageTutorial ~= nil then
		self.pageTutorial:setMissionInfo(isTutorialMission, missionInfo)
	end
...
function InGameMenu:setManureTriggers(manureHeaps, liquidManureTriggers)
	if self.pageSettingsGame ~= nil then
		self.pageSettingsGame:setManureTriggers(manureHeaps, liquidManureTriggers)
	end
end

function InGameMenu:setHusbandries(husbandries)
	self.pageAnimals:setHusbandries(husbandries)
end
...
function InGameMenu:onButtonSaveGame()
	...
	if self.missionInfo:isa(FSCareerMissionInfo) then
		if self.doSaveGameState == InGameMenu.SAVE_STATE_NONE then
		...
		g_gui:showYesNoDialog({
			text = self.l10n:getText(InGameMenu.L10N_SYMBOL.TUTORIAL_NOT_SAVED),
			callback = self.onYesNoRestartTutorial,
			target = self
		})
	end
end
...
function InGameMenu:update(dt)
	...
	if GS_PLATFORM_TYPE == GS_PLATFORM_TYPE_PS4 and g_currentMission ~= nil and self.missionDynamicInfo.isMultiplayer and getMultiplayerAvailability() == MultiplayerAvailability.NOT_AVAILABLE and self.continueEnabled then
...
function InGameMenu:updateButtonsPanel(page)
	local buttonsDisabled = page == self.pageMapOverview
...
function InGameMenu:onUnloadingStationsChanged()
	self:setSellingStations(g_currentMission.storageSystem:getSellingStations())
end
...
function InGameMenu:getTabBarProfile()
	if self.currentPage == self.pageMapOverview then
...
function InGameMenu:onYesNoRestartTutorial(yes)
	if yes then
		local currentTutorial = g_currentMission.missionInfo.id

		self:leaveCurrentGame()
		g_tutorialScreen:startTutorial(currentTutorial)
	end
end
...
function InGameMenu:onPageChange(pageIndex, pageMappingIndex, element, skipTabVisualUpdate)
	...
	if page == self.pageMapOverview then
		self.pageMapOverview:setStaticUIDeadzone(self.header.absPosition[1], self.header.absPosition[2], self.header.size[1], self.header.size[2])
		self.pageMapOverview:resetUIDeadzones()
...
function InGameMenu:getPageButtonInfo(page)
	...
	if self.isTutorialMission then
		buttonInfo = self.tutorialMenuButtonInfo
	end
...
function InGameMenu:makeIsTutorialEnabledPredicate()
	return function ()
		return self.isTutorialMission
	end
end

function InGameMenu:makeIsMapEnabledPredicate()
	return function ()
		return not self.isTutorialMission
	end
end

function InGameMenu:makeIsPricesEnabledPredicate()
	return function ()
		local isCareerMenuMode = not self.isTutorialMission
		...
		return isCareerMenuMode and isNotMultiplayerOrIsInFarm and not g_isPresentationVersion
	end
end
...
function InGameMenu:makeIsFinancesEnabledPredicate()
	return function ()
		...
		return isCareerMenuMode and isNotMultiplayerOrIsInFarm
	end
end

function InGameMenu:makeIsStatisticsEnabledPredicate()
	return function ()
		return not self.isTutorialMission and not self.missionDynamicInfo.isMultiplayer
	end
end
...
function InGameMenu:makeIsGameSettingsEnabledPredicate()
	return function ()
		...
		return not self.isTutorialMission and canChangeSettings
	end
end
...
function InGameMenu:makeIsControlsSettingsEnabledPredicate()
	return function ()
		return not self.isTutorialMission and not self.isConsoleVersion and not g_isPresentationVersion
	end
end
...
function InGameMenu:makeIsMpUsersEnabledPredicate()
	return function ()
		...
		return isCareerMenuMode and isMultiplayer
	end
end

function InGameMenu:makeIsMpFarmsEnabledPredicate()
	return function ()
		return not self.isTutorialMission and self.missionDynamicInfo.isMultiplayer
	end
end

function InGameMenu:makeIsHelpEnabledPredicate()
	return function ()
		return not self.isTutorialMission and not g_isPresentationVersion
	end
end
