...
function FocusManager:loadElementFromXML(xmlFile, xmlBaseNode, element)
	...
	FocusManager.allElements[element] = self.currentGui
...
function FocusManager:removeElement(element)
	...
	if FocusManager.allElements[element] ~= nil then
		local guiItWasAddedTo = FocusManager.allElements[element]

		if self.currentGui ~= guiItWasAddedTo then
			self.guiFocusData[guiItWasAddedTo].idToElementMapping[element.focusId] = nil
		end
...
function FocusManager:inputEvent(action, value, eventUsed)
	...
	pressedUp = action == InputAction.MENU_AXIS_UP_DOWN and g_analogStickVTolerance < value
	pressedDown = action == InputAction.MENU_AXIS_UP_DOWN and value < -g_analogStickVTolerance
	pressedLeft = action == InputAction.MENU_AXIS_LEFT_RIGHT and value < -g_analogStickHTolerance
	pressedRight = action == InputAction.MENU_AXIS_LEFT_RIGHT and g_analogStickHTolerance < value
...
function FocusManager:setHighlight(element)
	...
	if not self.currentFocusData.focusElement or self.currentFocusData.focusElement ~= element then
...
function FocusManager:unsetFocus(element, ...)
	...
	if prevFocusElement ~= element then
		return
	end
