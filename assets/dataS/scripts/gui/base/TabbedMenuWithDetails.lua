...
function TabbedMenuWithDetails:new(target, customMt, messageCenter, l10n, inputManager)
	local self = TabbedMenu:new(target, customMt or TabbedMenu_mt, messageCenter, l10n, inputManager)
	self.stacks = {}
...
function TabbedMenuWithDetails:isAtRoot()
	return table.getn(self:getStack()) == 1
end
...
function TabbedMenuWithDetails:popDetail()
	...
	if #stack == 1 then
		g_logManager:error("Cannot pop from view stack at root")
	...
	self:onDetailClosed(detailPage)
