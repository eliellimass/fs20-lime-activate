...
function Overlay:render()
	if self.visible then
		...
		if self.debugEnabled or g_uiDebugEnabled then
			...
			renderOverlay(GuiElement.debugOverlay, self.x + self.offsetX - xPixel, self.y + self.offsetY - yPixel, self.width + 2 * xPixel, yPixel)
			renderOverlay(GuiElement.debugOverlay, self.x + self.offsetX - xPixel, self.y + self.offsetY + self.height, self.width + 2 * xPixel, yPixel)
			renderOverlay(GuiElement.debugOverlay, self.x + self.offsetX - xPixel, self.y + self.offsetY, xPixel, self.height)
			renderOverlay(GuiElement.debugOverlay, self.x + self.offsetX + self.width, self.y + self.offsetY, xPixel, self.height)
		end
	end
end
...
function Overlay:setAlignment(vertical, horizontal)
	...
	self.alignmentVertical = Utils.getNoNil(vertical, Overlay.ALIGN_VERTICAL_BOTTOM)
	...
	self.alignmentHorizontal = Utils.getNoNil(horizontal, Overlay.ALIGN_HORIZONTAL_LEFT)
end
