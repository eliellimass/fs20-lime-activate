...
function CreateGameScreen:onCreate()
	self.portElement:setVisible(not GS_IS_CONSOLE_VERSION)
	self.useUpnpElement:setVisible(not GS_IS_CONSOLE_VERSION)
	self.bandwidthElement:setVisible(not GS_IS_CONSOLE_VERSION)
	self.allowOnlyFriendsElement:setVisible(GS_IS_CONSOLE_VERSION)
...
function CreateGameScreen:onOpen()
	...
	if GS_IS_CONSOLE_VERSION then
		if self.lastUserName ~= g_gameSettings:getValue("nickname") then
	...
	if reloadSettings then
		...
		self.autoSave = Utils.getNoNil(g_gameSettings:getTableValue("createGame", "autoSave"), false)

		self.autoSaveElement:setIsChecked(self.autoSave)
...
function CreateGameScreen:onClickAutoSave(element)
	self.autoSave = self.autoSaveElement:getIsChecked()
end
...
function CreateGameScreen:onClickOk()
	if not self.isTyping and self.blockTime <= self.time then
		...
		g_gameSettings:setTableValue("createGame", "autoSave", self.autoSave)
		...
		self.missionDynamicInfo.autoAccept = self.autoAccept or g_dedicatedServerInfo ~= nil
		...
		if GS_PLATFORM_TYPE == GS_PLATFORM_TYPE_PS4 then
			local ok, down, up, measured = netGetBandwidthEstimate()
...
function CreateGameScreen:verifyServerName()
	local serverName = StringUtil.trim(self.serverNameElement.text)
