...
InGameMenuFinancesFrame.PAST_DAY_COUNT = GS_IS_MOBILE_VERSION and 3 or 4
InGameMenuFinancesFrame.MAX_ITEMS = GS_IS_MOBILE_VERSION and 4 or 10
InGameMenuFinancesFrame.LOAN_STEP = 5000

function InGameMenuFinancesFrame:new(subclass_mt, messageCenter, l10n, inputManager)
	local subclass_mt = subclass_mt or InGameMenuFinancesFrame_mt
...
function InGameMenuFinancesFrame:update(dt)
	...
	if not g_currentMission:getIsServer() and self.updateTimeFinancesStats < g_currentMission.time then
		...
		if farm.stats.financesHistoryVersionCounter ~= farm.stats.financesHistoryVersionCounterLocal then
			...
			for i = 1, InGameMenuFinancesFrame.PAST_DAY_COUNT do
...
function InGameMenuFinancesFrame:getPastDays()
	local days = {}
	local currentDay = 1 + math.mod(self.environment.currentDay - 1, 7)

	for i = 1, InGameMenuFinancesFrame.PAST_DAY_COUNT do
		local pastDay = currentDay - i

		if pastDay <= 0 then
			pastDay = 7 + pastDay
		end

		local daySymbol = InGameMenuFinancesFrame.L10N_SYMBOL.WEEK_DAY_TEMPLATE .. tostring(pastDay)

		table.insert(days, self.l10n:getText(daySymbol))
	end
...
function InGameMenuFinancesFrame:updateDayTotals(currentFinances, pastFinances)
	for i = 1, InGameMenuFinancesFrame.PAST_DAY_COUNT + 1 do
...
function InGameMenuFinancesFrame:updateFinancesTable(currentFinances, pastFinances)
	...
	for i, statName in ipairs(currentFinances.statNames) do
...
function InGameMenuFinancesFrame:updateFinances()
	local pastDayLabels = self:getPastDays()
...
function InGameMenuFinancesFrame:updateMoneyUnit()
	...
	local text = string.gsub(repayTemplate, InGameMenuFinancesFrame.L10N_SYMBOL.CURRENCY, self.currentMoneyUnitText)
