...
function SettingsControlsFrame:new(subclass_mt, l10n)
	local subclass_mt = subclass_mt or SettingsControlsFrame_mt
...
function SettingsControlsFrame:initialize(controlsController, inGame)
	...
	if not inGame then
		self.keyboardMousePage:applyProfile("settingsControlsPageDark")
		self.gamepadPage:applyProfile("settingsControlsPageDark")
	end
end
...
function SettingsControlsFrame:updateHeader()
	...
	self.gamepadHeaderIcon:setVisible(hasGamepads)
...
function SettingsControlsFrame:setupControlsView()
	...
	self.keyboardMouseTable:initialize()
...
function SettingsControlsFrame:assignDeviceTableData()
	...
	for deviceCategory, dataBindings in pairs(self.deviceCategoryDataBindings) do
		local displayActionBindings = self.controlsController:getDeviceCategoryActionBindings(deviceCategory)
		local deviceTable = self.deviceCategoryTables[deviceCategory]

		deviceTable:clearData()

		for _, actionBinding in ipairs(displayActionBindings) do
			local dataRow = TableElement.DataRow:new(actionBinding.displayName, dataBindings)

			if deviceCategory == InputDevice.CATEGORY.KEYBOARD_MOUSE then
				dataRow.columnCells[dataBindings.ACTION].text = actionBinding.displayName
				dataRow.columnCells[dataBindings.ACTION].value = actionBinding
				dataRow.columnCells[dataBindings.KEY_1].text = actionBinding.columnTexts[ControlsController.BINDING_PRIMARY]
				dataRow.columnCells[dataBindings.KEY_2].text = actionBinding.columnTexts[ControlsController.BINDING_SECONDARY]
				dataRow.columnCells[dataBindings.MOUSE_BUTTON].text = actionBinding.columnTexts[ControlsController.BINDING_TERTIARY]
			else
				dataRow.columnCells[dataBindings.ACTION].text = actionBinding.displayName
				dataRow.columnCells[dataBindings.ACTION].value = actionBinding
				dataRow.columnCells[dataBindings.BUTTON_1].text = actionBinding.columnTexts[ControlsController.BINDING_PRIMARY]
				dataRow.columnCells[dataBindings.BUTTON_2].text = actionBinding.columnTexts[ControlsController.BINDING_SECONDARY]
			end

			deviceTable:addRow(dataRow)
		end

		deviceTable:updateView()
	end
end
...
function SettingsControlsFrame:notifyInputGatheringFinished(madeChange)
	...
	self.activeControlsTable:scrollTo(self.previousFirstRowIndex)
end
...
function SettingsControlsFrame:getMainElementSize()
	return self.pagingElement.size
end

function SettingsControlsFrame:getMainElementPosition()
	return self.pagingElement.absPosition
end
...
function SettingsControlsFrame:getActionDataFromClickedTableButton(tableButton, isKeyboard)
	local colName, clickedDataRow = nil

	if isKeyboard then
		colName = SettingsControlsFrame.KB_MOUSE_BOUND_CONTROLS.ACTION
	else
		colName = SettingsControlsFrame.GAMEPAD_BOUND_CONTROLS.ACTION
	end

	local clickedDataRow = self.activeControlsTable:getDataRowForElement(tableButton)

	return clickedDataRow.columnCells[colName].value
end
...
function SettingsControlsFrame:onClickKey1(element)
	self:onInputClicked(InputDevice.CATEGORY.KEYBOARD_MOUSE, ControlsController.BINDING_PRIMARY, self:getActionDataFromClickedTableButton(element, true), element)
end

function SettingsControlsFrame:onClickKey2(element)
	self:onInputClicked(InputDevice.CATEGORY.KEYBOARD_MOUSE, ControlsController.BINDING_SECONDARY, self:getActionDataFromClickedTableButton(element, true), element)
end

function SettingsControlsFrame:onClickMouse(element)
	self:onInputClicked(InputDevice.CATEGORY.KEYBOARD_MOUSE, ControlsController.BINDING_TERTIARY, self:getActionDataFromClickedTableButton(element, true), element)
end

function SettingsControlsFrame:onClickGamepadButton1(element)
	self:onInputClicked(InputDevice.CATEGORY.GAMEPAD, ControlsController.BINDING_PRIMARY, self:getActionDataFromClickedTableButton(element, false), element)
end

function SettingsControlsFrame:onClickGamepadButton2(element)
	self:onInputClicked(InputDevice.CATEGORY.GAMEPAD, ControlsController.BINDING_SECONDARY, self:getActionDataFromClickedTableButton(element, false), element)
end
