...
function DebugUtil.drawDebugNode(node, text, alignToGround)
	local x, y, z = getWorldTranslation(node)
	local upX, upY, upZ = localDirectionToWorld(node, 0, 1, 0)
	local dirX, dirY, dirZ = localDirectionToWorld(node, 0, 0, 1)

	DebugUtil.drawDebugGizmoAtWorldPos(x, y, z, dirX, dirY, dirZ, upX, upY, upZ, text, alignToGround)
end

function DebugUtil.drawDebugGizmoAtWorldPos(x, y, z, dirX, dirY, dirZ, upX, upY, upZ, text, alignToGround)
	...
	if text ~= nil then
		Utils.renderTextAtWorldPosition(x, y, z, tostring(text), getCorrectTextSize(0.012), 0)
	end
end

function DebugUtil.drawDebugArea(start, width, height, r, g, b, alignToGround, drawNodes, drawCircle)
	...
	if drawNodes == nil or drawNodes then
		DebugUtil.drawDebugNode(start, getName(start), alignToGround)
		DebugUtil.drawDebugNode(width, getName(width), alignToGround)
		DebugUtil.drawDebugNode(height, getName(height), alignToGround)
	end
...
function DebugUtil.drawDebugCube(node, sizeX, sizeY, sizeZ, r, g, b)
	sizeZ = sizeZ * 0.5
	sizeY = sizeY * 0.5
	sizeX = sizeX * 0.5
	local x, y, z = getWorldTranslation(node)
...
function DebugUtil.drawDebugParallelogram(x, z, widthX, widthZ, heightX, heightZ, heightOffset, r, g, b, a)
	local x0 = x
	local z0 = z
