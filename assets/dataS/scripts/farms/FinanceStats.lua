...
function FinanceStats:new(customMt)
	...
	setmetatable(self, customMt or FinanceStats_mt)
...
function FinanceStats:saveToXMLFile(xmlFile, key)
	for k, statName in ipairs(self.statNames) do
		setXMLFloat(xmlFile, key .. "." .. statName, self[statName])
	end
end

function FinanceStats:loadFromXMLFile(xmlFile, key)
	for _, statName in ipairs(self.statNames) do
		self[statName] = Utils.getNoNil(getXMLFloat(xmlFile, key .. "." .. statName), 0)
	end
end
