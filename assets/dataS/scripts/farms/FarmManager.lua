...
function FarmManager:loadMapData(xmlFile)
	...
	addConsoleCommand("gsSetFarm", "Set farm for current player or vehicle", "consoleCommandSetFarm", self)
end

function FarmManager:unloadMapData()
	g_currentMission:removeUpdateable(self)
	removeConsoleCommand("gsSetFarm")
...
function FarmManager:saveToXMLFile(xmlFilename)
	local xmlFile = createXMLFile("farmsXML", xmlFilename, "farms")
	local index = 0

	for i, farm in ipairs(self.farms) do
		if farm.farmId ~= 0 then
			local key = string.format("farms.farm(%d)", index)

			farm:saveToXMLFile(xmlFile, key)
...
function FarmManager:mergeObjectsForSingleplayer()
	if not g_currentMission.missionDynamicInfo.isMultiplayer and self.spFarmWasMerged then
		...
		for item, _ in pairs(g_currentMission.itemsToSave) do
...
function FarmManager:onFarmObjectDeleted(object)
	...
	g_messageCenter:publish(MessageType.FARM_DELETED, object.farmId)
end
