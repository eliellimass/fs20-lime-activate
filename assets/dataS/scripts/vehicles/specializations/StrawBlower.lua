...
function StrawBlower:onLoad(savegame)
	...
	XMLUtil.checkDeprecatedXMLElements(self.xmlFile, self.configFileName, "vehicle.strawBlower.baleTrigger#index", "vehicle.strawBlower.baleTrigger#node")
	XMLUtil.checkDeprecatedXMLElements(self.xmlFile, self.configFileName, "vehicle.strawBlower.doorAnimation#name", "vehicle.foldable.foldingParts.foldingPart.animationName")

	spec.triggeredBales = {}

	if self.isServer then
		spec.triggerId = I3DUtil.indexToObject(self.components, getXMLString(self.xmlFile, "vehicle.strawBlower.baleTrigger#node"), self.i3dMappings)

		if spec.triggerId ~= nil then
			addTrigger(spec.triggerId, "strawBlowerBaleTriggerCallback", self)
		end

		spec.pickupTriggerId = I3DUtil.indexToObject(self.components, getXMLString(self.xmlFile, "vehicle.strawBlower.balePickupTrigger#node"), self.i3dMappings)

		if spec.pickupTriggerId ~= nil then
			addTrigger(spec.pickupTriggerId, "strawBlowerBalePickupTriggerCallback", self)
		end

		spec.pickupTriggerSpawnNode = I3DUtil.indexToObject(self.components, getXMLString(self.xmlFile, "vehicle.strawBlower.balePickupTrigger#spawnNode"), self.i3dMappings)
		spec.pickupTriggerMaxBales = getXMLInt(self.xmlFile, "vehicle.strawBlower.balePickupTrigger#maxBales") or 2
	end
	...
	spec.fillUnitIndex = Utils.getNoNil(getXMLInt(self.xmlFile, "vehicle.strawBlower#fillUnitIndex"), 1)
...
function StrawBlower:onUpdateTick(dt, isActiveForInput, isActiveForInputIgnoreSelection, isSelected)
	...
	if self.isServer and spec.currentBale == nil and self:getFillUnitSupportsToolType(spec.fillUnitIndex, ToolType.BALE) then
