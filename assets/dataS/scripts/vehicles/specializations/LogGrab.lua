...
function LogGrab:onLoad(savegame)
	...
	if self.isServer then
		...
		while true do
			local baseKey = string.format("%s.grab(%d)", "vehicle.logGrab", i)
			...
			local entry = {
				componentJoint = getXMLInt(self.xmlFile, baseKey .. "#componentJoint"),
				dampingFactor = getXMLFloat(self.xmlFile, baseKey .. "#dampingFactor") or 20,
				axis = getXMLInt(self.xmlFile, baseKey .. "#axis") or 1,
				direction = {
					0,
					0,
					0
			...
			entry.rotationOffsetThreshold = Utils.getNoNilRad(getXMLFloat(self.xmlFile, baseKey .. "#rotationOffsetThreshold"), math.rad(10))
			entry.rotationOffsetTime = Utils.getNoNil(getXMLFloat(self.xmlFile, baseKey .. "#rotationOffsetTime"), 1000)
		...
		spec.jointNode = I3DUtil.indexToObject(self.components, getXMLString(self.xmlFile, "vehicle.logGrab#jointNode"), self.i3dMappings)
		spec.jointRoot = I3DUtil.indexToObject(self.components, getXMLString(self.xmlFile, "vehicle.logGrab#jointRoot"), self.i3dMappings)
		spec.lockAllAxis = Utils.getNoNil(getXMLBool(self.xmlFile, "vehicle.logGrab#lockAllAxis"), false)
		spec.triggerNode = I3DUtil.indexToObject(self.components, getXMLString(self.xmlFile, "vehicle.logGrab.trigger#node"), self.i3dMappings)
...
function LogGrab:logGrabTriggerCallback(triggerId, otherActorId, onEnter, onLeave, onStay, otherShapeId)
	...
	if onEnter then
		if getSplitType(otherActorId) ~= 0 then
			...
			if (rigidBodyType == "Dynamic" or rigidBodyType == "Kinematic") and spec.pendingDynamicMountShapes[otherActorId] == nil then
