...
function Tedder:onLoad(savegame)
	...
	XMLUtil.checkDeprecatedXMLElements(self.xmlFile, self.configFileName, "vehicle.turnedOnRotationNodes.turnedOnRotationNode#type", "vehicle.tedder.animationNodes.animationNode", "tedder")
	XMLUtil.checkDeprecatedXMLElements(self.xmlFile, self.configFileName, "vehicle.tedder.sounds", "vehicle.turnOnVehicle.sounds.work")

	spec.fillTypeConverters = {}
	spec.fillTypeConvertersReverse = {}
	local converter = getXMLString(self.xmlFile, "vehicle.tedder#fillTypeConverter")
...
function Tedder:onPostLoad(savegame)
	...
	for i = #spec.effects, 1, -1 do
		...
		if workArea ~= nil then
			...
			g_logManager:xmlWarning(self.xmlFileName, "Invalid workAreaIndex '%d' for effect 'vehicle.tedder.effects.effect(%d)'!", effect.workAreaIndex, i)
...
function Tedder:loadWorkAreaFromXML(superFunc, workArea, xmlFile, key)
	...
	if workArea.type == WorkAreaType.TEDDER then
		workArea.dropWindrowWorkAreaIndex = Utils.getNoNil(getXMLInt(xmlFile, key .. ".tedder#dropWindrowWorkAreaIndex"), 1)
...
function Tedder:processTedderArea(workArea, dt)
	...
	local lsx, lsy, lsz, lex, ley, lez, lineRadius = DensityMapHeightUtil.getLineByArea(workArea.start, workArea.width, workArea.height, true)
	...
	for targetFillType, inputFillTypes in pairs(spec.fillTypeConvertersReverse) do
		...
		if dropArea ~= nil and workArea.litersToDrop > 0 then
			...
			if self.isServer and dropped > 0 and self:getLastSpeed(true) > 0.5 then
