...
function TipOccluder:onLoad(savegame)
	...
	XMLUtil.checkDeprecatedXMLElements(self.xmlFile, self.configFileName, "vehicle.tipOcclusionAreas.tipOcclusionArea", "vehicle.tipOccluder.occlusionArea")
	...
	while true do
		...
		local entry = {
			start = I3DUtil.indexToObject(self.components, getXMLString(self.xmlFile, key .. "#start"), self.i3dMappings),
			width = I3DUtil.indexToObject(self.components, getXMLString(self.xmlFile, key .. "#width"), self.i3dMappings),
			height = I3DUtil.indexToObject(self.components, getXMLString(self.xmlFile, key .. "#height"), self.i3dMappings)
		}
...
function TipOccluder:onPostLoad()
	if self:getRequiresTipOcclusionArea() and table.getn(self.spec_tipOccluder.tipOcclusionAreas) == 0 then
		g_logManager:xmlDevWarning(self.configFileName, "No TipOcclusionArea defined")
	end
end
	...
	if wheel.tipOcclusionAreaGroupId ~= nil then
		...
		if area ~= nil then
			...
			for _, usedWheel in pairs(usedWheels) do
				local x, _, z = localToLocal(usedWheel.driveNode, vehicleRootNode, usedWheel.wheelshapeWidth - 0.5 * usedWheel.width, 0, -usedWheel.radius)
				xMax = math.max(x, xMax)
				zMin = math.min(z, zMin)
				x, _, z = localToLocal(usedWheel.driveNode, vehicleRootNode, -usedWheel.wheelshapeWidth + 0.5 * usedWheel.width, 0, usedWheel.radius)
