...
function TreePlanter:onLoad(savegame)
	...
	XMLUtil.checkDeprecatedXMLElements(self.xmlFile, self.configFileName, "vehicle.treePlanterSound", "vehicle.treePlanter.sounds.work")
	XMLUtil.checkDeprecatedXMLElements(self.xmlFile, self.configFileName, "vehicle.turnedOnRotationNodes.turnedOnRotationNode(0)", "vehicle.treePlanter.animationNodes.animationNode")
	...
	spec.node = I3DUtil.indexToObject(self.components, getXMLString(self.xmlFile, baseKey .. "#node"), self.i3dMappings)
	spec.minDistance = Utils.getNoNil(getXMLFloat(self.xmlFile, baseKey .. "#minDistance"), 20)
	spec.palletTrigger = I3DUtil.indexToObject(self.components, getXMLString(self.xmlFile, baseKey .. "#palletTrigger"), self.i3dMappings)

	if spec.palletTrigger ~= nil then
		addTrigger(spec.palletTrigger, "palletTriggerCallback", self)
	else
		g_logManager:xmlWarning(self.configFileName, "TreePlanter requires a palletTrigger!")
	end

	spec.palletsInTrigger = {}
	local refNodeIndex = Utils.getNoNil(getXMLInt(self.xmlFile, baseKey .. "#refNodeIndex"), 1)
	spec.groundReferenceNode = self:getGroundReferenceNodeFromIndex(refNodeIndex)

	if spec.groundReferenceNode == nil then
		g_logManager:xmlWarning(self.configFileName, "No groundReferenceNode specified or invalid groundReferenceNode index in '%s'", baseKey .. "#refNodeIndex")
	end

	spec.activatable = TreePlanterActivatable:new(self)
	spec.saplingPalletGrabNode = Utils.getNoNil(I3DUtil.indexToObject(self.components, getXMLString(self.xmlFile, baseKey .. "#saplingPalletGrabNode"), self.i3dMappings), self.rootNode)
	spec.saplingPalletMountNode = Utils.getNoNil(I3DUtil.indexToObject(self.components, getXMLString(self.xmlFile, baseKey .. "#saplingPalletMountNode"), self.i3dMappings), self.rootNode)
	spec.mountedSaplingPallet = nil
	spec.fillUnitIndex = Utils.getNoNil(getXMLInt(self.xmlFile, baseKey .. "#fillUnitIndex"), 1)
	spec.nearestPalletDistance = Utils.getNoNil(getXMLInt(self.xmlFile, baseKey .. "#palletMountingRange"), 6)
	spec.currentTree = 1
	...
	if savegame ~= nil and not savegame.resetVehicles then
		spec.lastTreePos = StringUtil.getVectorNFromString(getXMLString(savegame.xmlFile, savegame.key .. ".treePlanter#lastTreePos"), 3)
		spec.palletHadBeenMounted = getXMLBool(savegame.xmlFile, savegame.key .. ".treePlanter#palletHadBeenMounted")
	end
end
...
function TreePlanter:saveToXMLFile(xmlFile, key, usedModNames)
	...
	if spec.lastTreePos ~= nil then
		setXMLString(xmlFile, key .. "#lastTreePos", string.format("%f %f %f", unpack(spec.lastTreePos)))
	end

	if spec.mountedSaplingPallet ~= nil then
		setXMLBool(xmlFile, key .. "#palletHadBeenMounted", true)
	end
end
...
function TreePlanter:onUpdateTick(dt, isActiveForInput, isActiveForInputIgnoreSelection, isSelected)
	...
	if spec.hasGroundContact and self:getIsTurnedOn() and self.isServer then
		...
		if fillType == FillType.TREESAPLINGS then
			if self:getLastSpeed() > 1 then
				...
				if g_currentMission.accessHandler:canFarmAccessLand(self:getActiveFarm(), x, z) then
					if not PlacementUtil.isInsideRestrictedZone(g_currentMission.restrictedZones, nil, x, y, z) then
						if g_currentMission.waterY < y then
							if spec.lastTreePos ~= nil then
		...
		elseif fillType ~= FillType.UNKNOWN then
			...
			if g_currentMission.accessHandler:canFarmAccessLand(self:getActiveFarm(), x, z) then
				...
				FSDensityMapUtil.updateCultivatorArea(sx, sz, wx, wz, hx, hz, not limitToField, not limitGrassDestructionToField, angle, nil)
				...
				local area, _ = FSDensityMapUtil.updateSowingArea(fruitType, sx, sz, wx, wz, hx, hz, angle, 2)
				...
				stats:updateStats("workedHectares", lastHa)
...
function TreePlanter:getFillLevelInformation(superFunc, fillLevelInformations)
	local spec = self.spec_treePlanter
	local pallet = spec.mountedSaplingPallet
...
function TreePlanter:palletTriggerCallback(triggerId, otherId, onEnter, onLeave, onStay, otherShapeId)
	...
	if otherId ~= 0 then
		...
		if object ~= nil and object.isa ~= nil and object:isa(Vehicle) and object.typeName:find("pallet") ~= nil and g_currentMission.accessHandler:canFarmAccess(self:getActiveFarm(), object) then
...
function TreePlanterActivatable:new(treePlanterVehicle)
	...
	setmetatable(self, TreePlanterActivatable_mt)
...
function TreePlanterActivatable:getIsActivatable()
	if self.treePlanterVehicle:getRootVehicle() ~= g_currentMission.controlledVehicle then
		return false
	end
