...
function Pickup.registerEventListeners(vehicleType)
	...
	SpecializationUtil.registerEventListener(vehicleType, "onPostAttach", Pickup)
	SpecializationUtil.registerEventListener(vehicleType, "onPreDetach", Pickup)
end

function Pickup:onLoad(savegame)
	local spec = self.spec_pickup

	XMLUtil.checkDeprecatedXMLElements(self.xmlFile, self.configFileName, "vehicle.pickupAnimation", "vehicle.pickup.animation")

	spec.isLowered = false
	spec.animationName = Utils.getNoNil(getXMLString(self.xmlFile, "vehicle.pickup.animation#name"), "")
	...
	spec.animationLowerSpeed = Utils.getNoNil(getXMLFloat(self.xmlFile, "vehicle.pickup.animation#lowerSpeed"), 1)
	spec.animationLiftSpeed = Utils.getNoNil(getXMLFloat(self.xmlFile, "vehicle.pickup.animation#liftSpeed"), -spec.animationLowerSpeed)
	spec.animationIsDefaultLowered = Utils.getNoNil(getXMLBool(self.xmlFile, "vehicle.pickup.animation#isDefaultLowered"), false)
end
...
function Pickup:onPostAttach(attacherVehicle, inputJointDescIndex, jointDescIndex)
	local spec = self.spec_pickup

	if spec.animationName ~= "" then
		spec.controlledAction = self:getRootVehicle().actionController:registerAction("lowerPickup", InputAction.LOWER_IMPLEMENT, 2)
...
function Pickup:onPreDetach(attacherVehicle, implement)
	local spec = self.spec_pickup
...
function Pickup:getCanChangePickupState(newState)
	return true
end
...
function Pickup:updateActionEvents()
	...
	if spec.animationName ~= "" then
		...
		if actionEvent ~= nil then
			...
			g_inputBinding:setActionEventActive(actionEvent.actionEventId, self:getCanChangePickupState(not spec.isLowered))
		end
	end
end
...
function Pickup:actionEventTogglePickup(actionName, inputValue, callbackState, isAnalog)
	...
	if self:getCanChangePickupState(not spec.isLowered) then
