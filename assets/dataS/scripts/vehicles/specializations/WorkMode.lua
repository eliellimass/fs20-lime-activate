...
function WorkMode:onLoad(savegame)
	...
	spec.foldMaxLimit = Utils.getNoNil(getXMLFloat(self.xmlFile, string.format("%s#foldMaxLimit", baseKey)), 1)
	spec.foldMinLimit = Utils.getNoNil(getXMLFloat(self.xmlFile, string.format("%s#foldMinLimit", baseKey)), 0)
	spec.allowChangeOnLowered = Utils.getNoNil(getXMLBool(self.xmlFile, string.format("%s#allowChangeOnLowered", baseKey)), true)
	spec.workModes = {}
	local i = 0

	while true do
		local key = string.format("%s.workMode(%d)", baseKey, i)
		...
		while true do
			...
			local turnedOnAnimation = {
				name = getXMLString(self.xmlFile, key2 .. "#name"),
				turnOnFadeTime = Utils.getNoNil(getXMLFloat(self.xmlFile, key2 .. "#turnOnFadeTime"), 1) * 1000,
				turnOffFadeTime = Utils.getNoNil(getXMLFloat(self.xmlFile, key2 .. "#turnOffFadeTime"), 1) * 1000,
				speedScale = Utils.getNoNil(getXMLFloat(self.xmlFile, key2 .. "#speedScale"), 1),
				speedDirection = 0,
				currentSpeed = 0
			}
		...
		while true do
			...
			local loweringAnimation = {
				name = getXMLString(self.xmlFile, key2 .. "#name"),
				speed = Utils.getNoNil(getXMLFloat(self.xmlFile, key2 .. "#speed"), 1)
			}
		...
		while true do
			...
			local workArea = {
				workAreaIndex = Utils.getNoNil(getXMLInt(self.xmlFile, key2 .. "#workAreaIndex"), j + 1),
				dropAreaIndex = Utils.getNoNil(getXMLInt(self.xmlFile, key2 .. "#dropAreaIndex"), j + 1)
			}
		...
		while true do
			...
			local animation = {
				animName = getXMLString(self.xmlFile, animKey .. "#name"),
				animSpeed = Utils.getNoNil(getXMLFloat(self.xmlFile, animKey .. "#speed"), 1),
				stopTime = getXMLFloat(self.xmlFile, animKey .. "#stopTime"),
				repeatAfterUnfolding = Utils.getNoNil(getXMLBool(self.xmlFile, animKey .. "#repeatAfterUnfolding"), false),
				repeatStartTime = getXMLFloat(self.xmlFile, animKey .. "#repeatStartTime"),
				repeated = false
			}
...
function WorkMode:onPostLoad(savegame)
	if savegame ~= nil and not savegame.resetVehicles then
		...
		if spec.stateMax > 0 and hasXMLProperty(savegame.xmlFile, savegame.key .. ".workMode#state") then
			local workMode = Utils.getNoNil(getXMLInt(savegame.xmlFile, savegame.key .. ".workMode#state"), 1)
...
function WorkMode:onReadUpdateStream(streamId, timestamp, connection)
	if connection:getIsServer() and streamReadBool(streamId) then
		...
		spec.accumulatedFruitType = streamReadUIntN(streamId, 6)
	end
end
...
function WorkMode:onWriteUpdateStream(streamId, connection, dirtyMask)
	if not connection:getIsServer() then
		...
		if streamWriteBool(streamId, bitAND(dirtyMask, spec.dirtyFlag) ~= 0) then
			...
			streamWriteUIntN(streamId, spec.accumulatedFruitType, 6)
		end
	end
end
	...
	if self.isClient then
		...
		if spec.stateMax > 0 then
			...
			if isActiveForInputIgnoreSelection then
				local _, actionEventId = self:addActionEvent(spec.actionEvents, InputAction.TOGGLE_WORKMODE, self, WorkMode.actionEventWorkModeChange, false, true, false, true, nil)

				g_inputBinding:setActionEventTextPriority(actionEventId, GS_PRIO_NORMAL)

				for _, mode in ipairs(spec.workModes) do
					if mode.inputAction ~= nil then
						_, actionEventId = self:addActionEvent(spec.actionEvents, mode.inputAction, self, WorkMode.actionEventWorkModeChangeDirect, false, true, false, true, nil)
