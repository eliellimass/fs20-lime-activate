...
function Suspensions:onLoad(savegame)
	if self.isClient then
		...
		while true do
			...
			entry.useCharacterTorso = Utils.getNoNil(getXMLBool(self.xmlFile, key .. "#useCharacterTorso"), false)

			if entry.node ~= nil and entry.component ~= nil or entry.useCharacterTorso then
				entry.weight = Utils.getNoNil(getXMLFloat(self.xmlFile, key .. "#weight"), 500)
				entry.minRotation = StringUtil.getRadiansFromString(getXMLString(self.xmlFile, key .. "#minRotation"), 3)
				entry.maxRotation = StringUtil.getRadiansFromString(getXMLString(self.xmlFile, key .. "#maxRotation"), 3)
				...
				if not entry.isRotational and not entry.useCharacterTorso then
					...
					entry.minTranslation = StringUtil.getVectorNFromString(getXMLString(self.xmlFile, key .. "#minTranslation"), 3)
					entry.maxTranslation = StringUtil.getVectorNFromString(getXMLString(self.xmlFile, key .. "#maxTranslation"), 3)
				end

				entry.maxVelocityDifference = Utils.getNoNil(getXMLFloat(self.xmlFile, key .. "#maxVelocityDifference"), 0.1)
				local suspensionParametersX = StringUtil.getVectorNFromString(Utils.getNoNil(getXMLString(self.xmlFile, key .. "#suspensionParametersX"), "0 0"), 2)
				local suspensionParametersY = StringUtil.getVectorNFromString(Utils.getNoNil(getXMLString(self.xmlFile, key .. "#suspensionParametersY"), "0 0"), 2)
				local suspensionParametersZ = StringUtil.getVectorNFromString(Utils.getNoNil(getXMLString(self.xmlFile, key .. "#suspensionParametersZ"), "0 0"), 2)
				...
				entry.inverseMovement = Utils.getNoNil(getXMLBool(self.xmlFile, key .. "#inverseMovement"), false)
		...
		if spec.suspensionAvailable then
			...
			for _, suspension in ipairs(spec.suspensionNodes) do
				...
				elseif suspension.node ~= nil then
					g_logManager:xmlError(self.configFileName, "Failed to update suspension node %d. Node does not exist anymore!", suspension.node)
