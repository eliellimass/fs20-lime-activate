TreeSaw = {
	...
	onLoad = function (self, savegame)
		...
		XMLUtil.checkDeprecatedXMLElements(self.xmlFile, self.configFileName, "vehicle.turnedOnRotationNodes.turnedOnRotationNode", "vehicle.treeSaw.animationNodes.animationNode", "stumbCutter")
		XMLUtil.checkDeprecatedXMLElements(self.xmlFile, self.configFileName, "vehicle.treeSaw.cutParticleSystems.emitterShape(0)", "vehicle.treeSaw.effects.effectNode")
		XMLUtil.checkDeprecatedXMLElements(self.xmlFile, self.configFileName, "vehicle.treeSaw.sawSound", "vehicle.treeSaw.sounds.saw")
		XMLUtil.checkDeprecatedXMLElements(self.xmlFile, self.configFileName, "vehicle.treeSaw.cutSound", "vehicle.treeSaw.sounds.cut")
		...
		spec.cutNode = I3DUtil.indexToObject(self.components, getXMLString(self.xmlFile, baseKey .. ".cutNode#node"), self.i3dMappings)
		spec.cutSizeY = Utils.getNoNil(getXMLFloat(self.xmlFile, baseKey .. ".cutNode#sizeY"), 1)
		spec.cutSizeZ = Utils.getNoNil(getXMLFloat(self.xmlFile, baseKey .. ".cutNode#sizeZ"), 1)
		spec.lengthAboveThreshold = Utils.getNoNil(getXMLFloat(self.xmlFile, baseKey .. ".cutNode#lengthAboveThreshold"), 0.3)
		spec.lengthBelowThreshold = Utils.getNoNil(getXMLFloat(self.xmlFile, baseKey .. ".cutNode#lengthBelowThreshold"), 0.3)
		spec.cutTimerDuration = Utils.getNoNil(getXMLFloat(self.xmlFile, baseKey .. ".cutNode#timer"), 1) * 1000
		...
		if self:getIsTurnedOn() and spec.cutNode ~= nil then
			...
			if spec.curSplitShape ~= nil then
				...
				if minY == nil then
					...
					local _, y, _ = localToLocal(spec.cutNode, spec.curSplitShape, 0, minY, minZ)
					cutTooLow = cutTooLow or y < 0.01
					local _, y, _ = localToLocal(spec.cutNode, spec.curSplitShape, 0, minY, maxZ)
					cutTooLow = cutTooLow or y < 0.01
					local _, y, _ = localToLocal(spec.cutNode, spec.curSplitShape, 0, maxY, minZ)
					cutTooLow = cutTooLow or y < 0.01
					local _, y, _ = localToLocal(spec.cutNode, spec.curSplitShape, 0, maxY, maxZ)
					cutTooLow = cutTooLow or y < 0.01
