...
function AIDriveStrategy:setAIVehicle(vehicle)
	...
	self.lastValidGroundPosX, self.lastValidGroundPosY, self.lastValidGroundPosZ = getWorldTranslation(self.vehicle:getAIVehicleDirectionNode())
	self.lastHasNoField = false
end
...
function AIDriveStrategy:getDistanceToEndOfField(dt, vX, vY, vZ)
	...
	if self.fieldEndGabDetected then
		return 0, false, true
	end
	...
	for i = 1, table.getn(attachedAIImplements) do
		...
		if area > 0 then
			local farmId = self.vehicle.spec_aiVehicle.startedFarmId

			if g_farmlandManager:getIsOwnedByFarmAtWorldPosition(farmId, (rX + hX) * 0.5, (rZ + hZ) * 0.5) or g_missionManager:getIsMissionWorkAllowed(farmId, (rX + hX) * 0.5, (rZ + hZ) * 0.5, nil) then
				hasField = true
	...
	if hasField then
		...
		local dirX, dirY, dirZ = localDirectionToWorld(self.vehicle:getAIVehicleDirectionNode(), 0, 0, distance + 0.75)
