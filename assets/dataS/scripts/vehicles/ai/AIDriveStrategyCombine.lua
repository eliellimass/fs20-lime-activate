...
function AIDriveStrategyCombine:update(dt)
	for _, combine in pairs(self.combines) do
		if combine:getIsThreshingAllowed() and combine.spec_pipe ~= nil then
			local capacity = 0
			...
			if capacity == math.huge then
				...
				if rootVehicle.getAIIsTurning ~= nil and not rootVehicle:getAIIsTurning() then
...
function AIDriveStrategyCombine:getDriveData(dt, vX, vY, vZ)
	local rootVehicle = self.vehicle:getRootVehicle()
	local isTurning = rootVehicle.getAIIsTurning ~= nil and rootVehicle:getAIIsTurning()
