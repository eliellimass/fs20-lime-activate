		...
		if self.firstTimeRun then
			if allowedToDrive then
				...
				if doNotSteer == nil or not doNotSteer then
					...
					if maxSpeed * speedReduction < 1.5 then
						acceleration = 0
						speedReduction = 1.5 / maxSpeed
					end
...
function AIVehicleUtil.getMaxToolRadius(implement)
	local radius = 0
	local _, rotationNode, wheels = implement.object:getAITurnRadiusLimitation()
...
function AIVehicleUtil.updateInvertLeftRightMarkers(rootAttacherVehicle, vehicle)
	if vehicle.getAIMarkers ~= nil then
		...
		if leftMarker ~= nil and rightMarker ~= nil then
			local lX, _, _ = localToLocal(leftMarker, rootAttacherVehicle:getAIVehicleDirectionNode(), 0, 0, 0)
			local rX, _, _ = localToLocal(rightMarker, rootAttacherVehicle:getAIVehicleDirectionNode(), 0, 0, 0)
...
function AIVehicleUtil.getValidityOfTurnDirections(vehicle, turnData)
	local directionNode = vehicle:getAIVehicleDirectionNode()
	...
	leftAreaPercentage = leftAreaPercentage / table.getn(attachedAIImplements)
	rightAreaPercentage = rightAreaPercentage / table.getn(attachedAIImplements)
...
function AIVehicleUtil.getIsAreaOwned(vehicle, sX, sZ, wX, wZ, hX, hZ)
	local farmId = vehicle.spec_aiVehicle.startedFarmId
...
function AIVehicleUtil.getAIAreaOfVehicle(vehicle, startWorldX, startWorldZ, widthWorldX, widthWorldZ, heightWorldX, heightWorldZ)
	local useDensityHeightMap, useWindrowFruitType = vehicle:getAIFruitExtraRequirements()

	if not useDensityHeightMap then
		local query = vehicle:getFieldCropsQuery()

		return AIVehicleUtil.getAIFruitArea(startWorldX, startWorldZ, widthWorldX, widthWorldZ, heightWorldX, heightWorldZ, query)
	else
		local fruitRequirements = vehicle:getAIFruitRequirements()

		return AIVehicleUtil.getAIDensityHeightArea(startWorldX, startWorldZ, widthWorldX, widthWorldZ, heightWorldX, heightWorldZ, fruitRequirements, useWindrowFruitType)
	end
end
...
function AIVehicleUtil.getAIDensityHeightArea(startWorldX, startWorldZ, widthWorldX, widthWorldZ, heightWorldX, heightWorldZ, fruitRequirements, useWindrowed)
