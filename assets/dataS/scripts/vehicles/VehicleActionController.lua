...
function VehicleActionController:saveToXMLFile(xmlFile, key, usedModNames)
	if #self.actions > 0 then
		setXMLInt(xmlFile, key .. "#lastDirection", self.lastDirection)
		setXMLInt(xmlFile, key .. "#numActions", #self.actions)
		...
		for _, action in ipairs(self.actions) do
			if action:getIsSaved() then
				...
				setXMLString(xmlFile, actionKey .. "#name", action.name)
				setXMLString(xmlFile, actionKey .. "#identifier", action.identifier)
				setXMLInt(xmlFile, actionKey .. "#lastDirection", action:getLastDirection())
...
function VehicleActionController:load(savegame)
	if savegame ~= nil then
		self.lastDirection = Utils.getNoNil(getXMLInt(savegame.xmlFile, savegame.key .. ".actionController#lastDirection"), self.lastDirection)
		self.loadedNumActions = getXMLInt(savegame.xmlFile, savegame.key .. ".actionController#numActions") or 0
		...
		while true do
			...
			local action = {
				name = getXMLString(savegame.xmlFile, baseKey .. "#name"),
				identifier = getXMLString(savegame.xmlFile, baseKey .. "#identifier"),
				lastDirection = getXMLInt(savegame.xmlFile, baseKey .. "#lastDirection")
			}
...
function VehicleActionController:removeAction(action)
	if action:getLastDirection() == 1 and action:getDoResetOnDeactivation() then
		action:doAction()
	end
...
function VehicleActionController:updateSortedActions()
	...
	self.sortedActions = ListUtil.copyTable(self.actionsByPrio)
	...
	self.sortedActionsRev = ListUtil.copyTable(self.actionsByPrio)
...
function VehicleActionController:registerActionEvents(isActiveForInput, isActiveForInputIgnoreSelection)
	if #self.actions > 0 and self.vehicle:getRootVehicle() == self.vehicle then
