...
function BunkerSilo:onCreate(id)
	g_logManager:error("BunkerSilo.onCreate is deprecated!")
end
...
function BunkerSilo:new(isServer, isClient, customMt)
	...
	self.fermentingDuration = 21600000
...
function BunkerSilo:load(id, xmlFile, key)
	self.nodeId = id
	self.bunkerSiloArea.start = I3DUtil.indexToObject(id, getXMLString(xmlFile, key .. ".area#startNode"))
	self.bunkerSiloArea.width = I3DUtil.indexToObject(id, getXMLString(xmlFile, key .. ".area#widthNode"))
	self.bunkerSiloArea.height = I3DUtil.indexToObject(id, getXMLString(xmlFile, key .. ".area#heightNode"))
	...
	self.interactionTriggerNode = I3DUtil.indexToObject(id, getXMLString(xmlFile, key .. ".interactionTrigger#node"))
	...
	self.acceptedFillTypes = {}
	local data = StringUtil.splitString(" ", getXMLString(xmlFile, key .. "#acceptedFillTypes") or "chaff grass_windrow dryGrass_windrow")
	...
	for i = 1, table.getn(data) do
		...
		if fillTypeIndex ~= nil then
			...
			g_logManager:warning("'%s' is an invalid fillType for bunkerSilo '%s'!", tostring(data[i]), key .. "#acceptedFillTypes")
		end
	end

	local inputFillTypeName = getXMLString(xmlFile, key .. "#inputFillType") or "chaff"
	...
	if inputFillTypeIndex ~= nil then
		...
		g_logManager:warning("'%s' is an invalid input fillType for bunkerSilo '%s'!", tostring(inputFillTypeName), key .. "#inputFillType")
	end

	local outputFillTypeName = getXMLString(xmlFile, key .. "#outputFillType") or "silage"
	...
	if outputFillTypeIndex ~= nil then
		...
		g_logManager:warning("'%s' is an invalid output fillType for bunkerSilo '%s'!", tostring(outputFillTypeName), key .. "#outputFillType")
	end

	g_densityMapHeightManager:setConvertingFillTypeAreas(self.bunkerSiloArea, self.acceptedFillTypes, self.inputFillType)

	self.distanceToCompactedFillLevel = getXMLFloat(xmlFile, key .. "#distanceToCompactedFillLevel") or self.distanceToCompactedFillLevel
	self.fermentingDuration = (getXMLFloat(xmlFile, key .. "#fermentingHours") or 6) * 1000 * 60 * 60
	self.openingLength = getXMLFloat(xmlFile, key .. "#openingLength") or 5
	self.fillLevel = 0
	...
	self.isTutorialSilo = Utils.getNoNil(getXMLBool(xmlFile, key .. "#isTutorialSilo"), false)
...
function BunkerSilo:saveToXMLFile(xmlFile, key, usedModNames)
	...
	setXMLFloat(xmlFile, key .. "#compactedFillLevel", self.compactedFillLevel)
	setXMLFloat(xmlFile, key .. "#fermentingTime", self.fermentingTime)
	setXMLBool(xmlFile, key .. "#openedAtFront", self.isOpenedAtFront)
	setXMLBool(xmlFile, key .. "#openedAtBack", self.isOpenedAtBack)
end
		...
		for vehicle, state in pairs(self.vehiclesInRange) do
			if state and vehicle:getIsActive() then
				...
				if distance > 0 then
					...
					local refNode = vehicle.components[1].node
					local scale = mass / BunkerSilo.COMPACTING_BASE_MASS
					local bunkerSiloCompacter = nil

					if vehicle.getBunkerSiloCompacter ~= nil then
						bunkerSiloCompacter = vehicle:getBunkerSiloCompacter()
					end

					if bunkerSiloCompacter ~= nil then
						if bunkerSiloCompacter.refNode ~= nil then
							refNode = bunkerSiloCompacter.refNode
						end

						if bunkerSiloCompacter.scale ~= nil then
							scale = mass / BunkerSilo.COMPACTING_BASE_MASS * bunkerSiloCompacter.scale
						end
					end
...
function BunkerSilo:onChangedFillLevelCallback(vehicle, fillDelta, fillType)
	if fillDelta >= 0 then
		return
	end
	...
	local shovelSpec = vehicle.spec_shovel

	if shovelSpec ~= nil and #shovelSpec.shovelNodes > 0 then
		self.shovelNodeIndex = (self.shovelNodeIndex or 0) + 1

		if self.shovelNodeIndex > #shovelSpec.shovelNodes then
			self.shovelNodeIndex = 1
		end

		local shovelNode = shovelSpec.shovelNodes[self.shovelNodeIndex]

		if shovelNode ~= nil then
			x, y, z = localToWorld(shovelNode.node, 0, 0, shovelNode.zOffset + shovelNode.length)
		end
	end
...
function BunkerSiloActivatable:onActivateObject()
...
function BunkerSiloActivatable:shouldRemoveActivatable()
	return false
end
