...
function FillLevelListener:load(id)
	...
	self.minMaxY = StringUtil.getVectorNFromString(minMaxY, 2)
...
function FillLevelListener:setSource(source)
	...
	assert(source.getProvidedFillTypes ~= nil)

	self.source = source
	local providedFillTypes = self.source:getProvidedFillTypes()

	for fillType, _ in pairs(self.fillTypes) do
		if providedFillTypes[fillType] ~= true then
