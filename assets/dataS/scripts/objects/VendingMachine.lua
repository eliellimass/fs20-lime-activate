...
function VendingMachine:onCreate(id)
	table.insert(g_currentMission.updateables, VendingMachine:new(id))
end
...
function VendingMachine:update(dt)
	...
	if self.emitTimer ~= 0 and self.emitTimer < self.time then
		self.emitTimer = 0
		local rootNode = g_i3DManager:loadSharedI3DFile("data/maps/models/objects/can/can.i3d", "", true)
