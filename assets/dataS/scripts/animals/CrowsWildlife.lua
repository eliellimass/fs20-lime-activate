...
CrowsWildlife.CROW_STATES = {
	{
		id = "default",
		class = CrowStateDefault
	},
	{
		id = "fly_glide",
		class = CrowStateFlyGlide
	},
	{
		id = "fly",
		class = CrowStateFly
	...
	{
		id = "takeOff",
		class = CrowStateTakeOff
	},
	{
		id = "idle_walk",
		class = CrowStateIdleWalk
	},
	{
		id = "idle_eat",
		class = CrowStateIdleEat
	},
	{
		id = "idle_attention",
		class = CrowStateIdleAttention
	}
}
...
function CrowsWildlife:new(customMt)
	local self = LightWildlife:new(customMt or CrowsWildlife_mt)
	self.animalStates = {}
...
function CrowsWildlife:searchTree(x, y, z, radius)
	overlapSphere(x, y, z, radius, "treeSearchCallback", self, 2, false, true, false)
end
...
function CrowsWildlife:getAverageLocationOfIdleAnimals()
	...
	for _, animal in pairs(self.animals) do
		...
		if currentState == "idle_walk" or currentState == "idle_eat" or currentState == "idle_attention" then
			local posX, posY, posZ = getWorldTranslation(animal.i3dNodeId)
