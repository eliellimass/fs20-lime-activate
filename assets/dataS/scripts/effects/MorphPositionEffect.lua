...
function MorphPositionEffect:update(dt)
	...
	if self.currentDelay <= 0 and self.state ~= ShaderPlaneEffect.STATE_OFF then
		...
		setShaderParameter(self.node, "morphPosition", self.fadeCur[1], self.fadeCur[2], 1, self.speed, false)
...
function MorphPositionEffect:start()
...
function MorphPositionEffect:stop()
...
function MorphPositionEffect:reset()
	...
	setShaderParameter(self.node, "morphPosition", self.fadeCur[1], self.fadeCur[2], 0, self.scrollSpeed, false)
	setVisibility(self.node, false)
