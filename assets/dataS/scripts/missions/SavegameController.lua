...
function SavegameController:onSaveGameUpdateCompleteCloudError(errorCode)
	local conflictedSavegameId = self.cloudErrorConflictedSavegameId
	self.cloudErrorConflictedSavegameId = nil

	self:loadSavegames()

	if conflictedSavegameId ~= nil then
		self:tryToResolveConflict(conflictedSavegameId)
	end
end
...
function SavegameController:onCloudError(errorCode, savegameId)
	...
	if savegameId ~= nil and errorCode == Savegame.ERROR_CLOUD_CONFLICT then
		self.cloudErrorConflictedSavegameId = savegameId
...
function SavegameController:locateBackups(backupBasePath, backupDirBase)
	...
	for _, v in pairs(files.files) do
		if v.isDirectory and StringUtil.startsWith(v.filename, backupDirBase) then
	...
	if errorCode == Savegame.ERROR_OK and savegameDirectory ~= nil then
		...
		for _, fruit in pairs(g_currentMission.fruits) do
			if fruit.id ~= 0 then
				...
				if savedDensityMaps[filename] == nil then
					...
					if self.isSavingBlocking then
						...
						prepareSaveDensityMapToFile(fruit.id, dir .. "/" .. filename)
						self:addSaveTask(SavegameController.SAVE_TASK_DENSITY_MAP, fruit.id)
					end
				end
			end
			...
			if fruit.preparingOutputId ~= 0 then
				...
				if savedDensityMaps[filename] == nil then
					...
					if self.isSavingBlocking then
						saveDensityMapToFile(fruit.preparingOutputId, dir .. "/" .. filename)
					else
						prepareSaveDensityMapToFile(fruit.preparingOutputId, dir .. "/" .. filename)
						self:addSaveTask(SavegameController.SAVE_TASK_DENSITY_MAP, fruit.preparingOutputId)
					end
				end
			end
		end
		...
		if savedDensityMaps[detailFilename] == nil then
			...
			if self.isSavingBlocking then
				...
				prepareSaveDensityMapToFile(g_currentMission.terrainDetailId, dir .. "/" .. detailFilename)
				self:addSaveTask(SavegameController.SAVE_TASK_DENSITY_MAP, g_currentMission.terrainDetailId)
			end
		end
		...
		for filename, updater in pairs(g_currentMission.fieldCropsUpdaters) do
			saveCropsGrowthStateToFile(updater.updater, dir .. "/" .. filename .. "_growthState.xml")
		end

		if g_currentMission.weedUpdater ~= nil then
			saveTerrainDetailUpdaterStateToFile(g_currentMission.weedUpdater, dir .. "/weed_growthState.xml")
		end
