...
function HelperManager:loadHelpers(xmlFile, missionInfo, baseDirectory, isBaseType)
	...
	while true do
		...
		self:addHelper(name, title, filename, baseDirectory, isBaseType)
...
function HelperManager:addHelper(name, title, filename, baseDir, isBaseType)
	if g_buildTypeParam == "CHINA" or g_buildTypeParam == "CHINA_GAPP" then
		name = tostring(self.numHelpers + 1)
	end
	...
	if helper == nil then
		...
		helper.filename = Utils.getFilename(filename, baseDir)
