...
function NPCManager:loadDefaultTypes(missionInfo, baseDirectory)
	local xmlFile = loadXMLFile("nps", "data/maps/maps_npcs.xml")
...
function NPCManager:loadNPCs(xmlFile, missionInfo, baseDirectory, isBaseType)
	...
	while true do
		...
		self:addNPC(name, title, category, imageFilename, baseDirectory, isBaseType)
...
function NPCManager:addNPC(name, title, category, imageFilename, baseDir, isBaseType)
	...
	if npc == nil then
		...
		if category == nil or category == "" then
			print("Warning: '" .. tostring(category) .. "' is not a valid category for a npc. Ignoring npc!")
