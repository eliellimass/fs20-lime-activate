...
function PlayerStateMachine:new(player, custom_mt)
	...
	self.playerStateAnimalFeed = PlayerStateAnimalFeed:new(self.player, self)
	...
	self.playerStateActicateObject = PlayerStateActivateObject:new(self.player, self)
	...
	self.fsmTable.walk.activateObject = true
...
function PlayerStateMachine:delete()
	if self.player.isOwner then
		removeConsoleCommand("gsToggleDebugPlayerFSM")
	end
...
function PlayerStateMachine:load()
	...
	if self.player.isOwner then
		addConsoleCommand("gsToggleDebugPlayerFSM", "Toggle debug mode for player state machine", "consoleCommandDebugFinalStateMachine", self)
	end
end
