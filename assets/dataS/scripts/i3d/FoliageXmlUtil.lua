FoliageXmlUtil = {
	printFoliageCtor = function (ctor)
		...
		for i = 1, numLayers do
			...
			local name = ctor:getNameForLayer(layerIndex)
			local typeIndex = ctor:getTypeIndexForLayer(layerIndex)
			local shapeSource = ctor:getShapeSourceForLayer(layerIndex)
			local channelOffset, numChannels = ctor:getDensityChannelsForLayer(layerIndex)
			...
			print("  Density map: CH off " .. channelOffset .. " BPP " .. numChannels)
			...
			for j = 1, numStates do
				...
				local name = ctor:getNameForState(layerIndex, stateIndex)
