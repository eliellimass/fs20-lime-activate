...
function CameraPath:new(posAnimCurve, rotAnimCurve, speedAnimCurve, speedScale, camera, maxTime)
	local self = {}
...
function CameraPath.createFromI3D(filename, speedScale, camera)
	local rootNode = loadI3DFile(filename)
	...
	for i = 1, table.getn(posAnimCurve.keyframes) - 1 do
		...
		local d = 0
		local lastX, lastY, lastZ = posAnimCurve:getFromKeyframes(keyframe1, keyframe2, i, i + 1, 1)
