...
function SellHandToolEvent:run(connection)
	if not connection:getIsServer() then
		...
		if g_currentMission:getHasPlayerPermission("sellVehicle", connection) then
			for _, item in ipairs(g_storeManager:getItems()) do
				if item.xmlFilenameLower == filename then
					dataStoreItem = item
		...
		if dataStoreItem ~= nil and not toolInUse and g_currentMission.players ~= nil then
			...
			if g_currentMission:getIsServer() then
				g_server:broadcastEvent(SellHandToolEvent:new(self.filename, self.farmId), false, connection)
	...
	elseif self.isAnswer then
		if self.state == SellHandToolEvent.STATE_SUCCESS then
			self:removeHandTool(self.filename:lower(), self.farmId)
		end
