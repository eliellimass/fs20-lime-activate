...
function MasterServerConnection:onServerInfo(id, name, language, capacity, numPlayers, mapName, hasPassword, allModsAvailable, isLanServer, isFriendServer)
	if self.masterServerCallbackTarget.onServerInfo ~= nil then
		self.masterServerCallbackTarget:onServerInfo(id, name, language, capacity, numPlayers, mapName, hasPassword, allModsAvailable, isLanServer, isFriendServer)
	else
		g_logManager:devWarning("Warning: Callback target is missing onServerInfo")
	end
end
...
function MasterServerConnection:onServerInfoStart(numServers, totalNumServers)
	if self.masterServerCallbackTarget.onServerInfoStart ~= nil then
		...
		g_logManager:devWarning("Warning: Callback target is missing onServerInfoStart")
	end
end
...
function MasterServerConnection:onServerInfoEnd()
	if self.masterServerCallbackTarget.onServerInfoEnd ~= nil then
		...
		g_logManager:devWarning("Warning: Callback target is missing onServerInfoEnd")
	end
end
...
function MasterServerConnection:onServerInfoDetails(id, ip, port, name, language, capacity, numPlayers, mapName, mapId, hasPassword, isLanServer, modTitles, modHashs)
	if self.masterServerCallbackTarget.onServerInfoDetails ~= nil then
		...
		g_logManager:devWarning("Warning: Callback target is missing onServerInfoDetails")
	end
end
...
function MasterServerConnection:onServerInfoDetailsFailed()
	if self.masterServerCallbackTarget.onServerInfoDetailsFailed ~= nil then
		...
		g_logManager:devWarning("Warning: Callback target is missing onServerInfoDetailsFailed")
	end
end
...
function MasterServerConnection:init()
	if not self.isInit and not GS_IS_MOBILE_VERSION then
		...
		for _, mod in ipairs(g_modManager:getMultiplayerMods()) do
			if StringUtil.endsWith(mod.modFile, "dlcDesc.xml") then
