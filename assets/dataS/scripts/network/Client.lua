...
function Client:new()
	...
	self.tempClientManurallyRegisteringObjects = {}
	self.tickRate = 30
	...
	if g_server == nil then
		...
		addConsoleCommand("gsNetworkShowActiveObjects", "Toggle network show active server objects", "consoleCommandToggleNetworkShowActiveObjects", self)
	end
...
function Client:delete()
	if g_server == nil then
		...
		removeConsoleCommand("gsNetworkShowActiveObjects")
	end
	...
	for _, object in pairs(self.tempClientManurallyRegisteringObjects) do
		object:delete()
	end
		...
		if self.tickSum >= self.tickDuration - 3 then
			...
			if self.serverConnection:getIsWindowFull() then
				...
				for j = 1, numDirtyObjects do
					...
					local id = dirtyObjects[j].lastServerId

					NetworkUtil.writeNodeObjectId(self.serverStreamId, id)
...
function Client:packetReceived(packetType, timestamp, streamId)
	...
	if packetType == Network.TYPE_APPLICATION then
		local id = streamReadUIntN(streamId, MessageIds.SEND_NUM_BITS)

		if id == MessageIds.OBJECT_UPDATE then
		...
		elseif id == MessageIds.EVENT_IDS then
			...
			for i = 1, numIds do
				local id = streamReadUIntN(streamId, EventIds.SEND_NUM_BITS)
		...
		elseif id == MessageIds.OBJECT_CLASS_IDS then
			...
			for i = 1, numIds do
				local id = streamReadUIntN(streamId, ObjectIds.SEND_NUM_BITS)
				local className = streamReadString(streamId)
...
function Client:registerObject(object, alreadySent)
	if not object.isRegistered then
		...
		if alreadySent then
			self.tempClientManurallyRegisteringObjects[object.id] = object
		else
			if g_server ~= nil then
...
function Client:unregisterObject(object, alreadySent)
	if object.isRegistered then
		...
		if serverId ~= nil then
			...
			self.tempClientManurallyRegisteringObjects[object.id] = nil
