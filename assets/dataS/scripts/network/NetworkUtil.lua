...
function NetworkUtil.writeCompressedRange(streamId, value, minValue, maxValue, numBits)
	...
	local value = (math.min(math.max(value, minValue), maxValue) - minValue) / (maxValue - minValue)
...
function NetworkUtil.convertToNetworkFilename(filename)
	local modFilename, isMod, isDlc, _ = Utils.removeModDirectory(StringUtil.trim(filename))
...
function NetworkUtil.convertFromNetworkFilename(filename)
	...
	if StringUtil.startsWith(filenameLower, modPrefix) then
	...
	elseif StringUtil.startsWith(filenameLower, mapPrefix) then
		...
		if StringUtil.startsWith(filenameLower, pdlcPrefix) then
