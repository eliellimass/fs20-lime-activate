...
function MapPerformanceTestUtil:new()
	...
	addConsoleCommand("gsRunMapPerformanceTest", "Runs a basic performance test for the current map", "runMapPerformanceTest", self)
...
function MapPerformanceTestUtil:delete()
	...
	removeConsoleCommand("gsRunMapPerformanceTest")
end
			...
			if self.fileId ~= nil and self.fileId ~= 0 then
				for _, vehicle in pairs(g_currentMission.vehicles) do
					if vehicle.aiIsStarted then
						vehicle:stopAIVehicle(AIVehicle.STOP_REASON_REGULAR)
					end
				end
				...
				for x = 1, self.testProps.xStepCount do
					...
					for z = 1, self.testProps.zStepCount do
						local entry = {
							totalTime = {},
							totalFrames = {},
							meanFps = {}
...
function MapPerformanceTestUtil:writeTestDataToFile()
	if self.fileId ~= nil and self.fileId ~= 0 then
		...
		local terrainSize_2 = g_currentMission.terrainSize / 2
		local imageSize = 512
