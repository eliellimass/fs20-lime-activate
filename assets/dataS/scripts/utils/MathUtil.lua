...
function MathUtil.vector3SetLength(x, y, z, length)
	local normalizedVector = {
		MathUtil.vector3Normalize(x, y, z)
	}
	x = normalizedVector[1] * length
	y = normalizedVector[2] * length
	z = normalizedVector[3] * length
...
function MathUtil.hasRectangleLineIntersection2D(x1, z1, dirX1, dirZ1, dirX2, dirZ2, x3, z3, dirX3, dirZ3)
	...
	local intersects, t1, t2 = MathUtil.getLineLineIntersection2D(x3, z3, dirX3_norm, dirZ3_norm, x1, z1, dirX2_norm, dirZ2_norm)

	if intersects and t1 > 0 and t1 < dir3Length and t2 > 0 and t2 < dir2Length then
		return true
	end

	local intersects, t1, t2 = MathUtil.getLineLineIntersection2D(x3, z3, dirX3_norm, dirZ3_norm, x1 + dirX1, z1 + dirZ1, dirX2_norm, dirZ2_norm)

	if intersects and t1 > 0 and t1 < dir3Length and t2 > 0 and t2 < dir2Length then
		return true
	end

	local intersects, t1, t2 = MathUtil.getLineLineIntersection2D(x3, z3, dirX3_norm, dirZ3_norm, x1 + dirX2, z1 + dirZ2, dirX1_norm, dirZ1_norm)
	...
	if p1 > 0 and p1 < dir1Length and p2 > 0 and p2 < dir2Length then
		local p1 = MathUtil.getProjectOnLineParameter(x3 + dirX3, z3 + dirZ3, x1, z1, dirX1_norm, dirZ1_norm)
		local p2 = MathUtil.getProjectOnLineParameter(x3 + dirX3, z3 + dirZ3, x1, z1, dirX2_norm, dirZ2_norm)
...
function MathUtil.getClosestPointOnLineSegment(startX, startY, startZ, endX, endY, endZ, targetX, targetY, targetZ)
	...
	elseif distance > 1 then
		...
		return startX + dirLineX * distance, startY + dirLineY * distance, startZ + dirLineZ * distance
	end
end
