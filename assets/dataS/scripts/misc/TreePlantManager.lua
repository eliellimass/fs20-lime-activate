...
function TreePlantManager:loadDefaultTypes(missionInfo, baseDirectory)
	...
	self:loadTreeTypes(xmlFile, missionInfo, true)
	delete(xmlFile)
end
...
function TreePlantManager:loadMapData(xmlFile, missionInfo, baseDirectory)
	...
	return XMLUtil.loadDataFromMapXML(xmlFile, "treeTypes", baseDirectory, self, self.loadTreeTypes, missionInfo)
end
...
function TreePlantManager:loadTreeTypes(xmlFile, missionInfo, isBaseType)
	local i = 0
...
function TreePlantManager:plantTree(treeType, x, y, z, rx, ry, rz, growthState, growthStateI, isGrowing, splitShapeFileId)
	...
	if treeTypeDesc ~= nil then
		...
		local isGrowing = Utils.getNoNil(isGrowing, true)
		...
		g_server:broadcastEvent(TreePlantEvent:new(treeType, x, y, z, rx, ry, rz, growthState, splitShapeFileId))
	end
end

function TreePlantManager:loadTreeNode(treeTypeDesc, x, y, z, rx, ry, rz, growthStateI, splitShapeFileId)
	local treesData = self.treesData
	...
	setSplitShapesLoadingFileId(Utils.getNoNil(splitShapeFileId, -1))

	local splitShapeFileId = setSplitShapesNextFileId()
	local treeId = g_i3DManager:loadSharedI3DFile(i3dFilename, nil, false, false)
...
function TreePlantManager:updateTrees(dt, dtGame)
	...
	if treesData.updateDtGame > 600000 then
		self:cleanupDeletedTrees()

		local dtHours = treesData.updateDtGame / 3600000
