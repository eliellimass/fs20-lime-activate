GamingStationManager = {
	...
	STANDALONE_CONFIGURATION_FILE = "dataS/gamingStation/standalone.xml",
	LOAD_NONE = 0,
	LOAD_GS = 1,
	LOAD_STANDALONE = 2,
...
function GamingStationManager:new(customMt)
	self = AbstractManager:new(customMt or GamingStationManager_mt)
	...
	if self.loadState == GamingStationManager.LOAD_GS or self.loadState == GamingStationManager.LOAD_STANDALONE then
		...
		if self.loadState == GamingStationManager.LOAD_GS then
			self.brandId = self:getCurrentBrand()
		else
			xmlFile = loadXMLFile("standaloneConfigConfig", GamingStationManager.STANDALONE_CONFIGURATION_FILE)

			self:loadConfigurationFile(xmlFile)

			self.brandId = getXMLInt(xmlFile, "standalone#brandId")

			delete(xmlFile)

			if self.brandId == nil then
				g_logManager:error("Could not find standalone brandId in '%s'", GamingStationManager.STANDALONE_CONFIGURATION_FILE)
			end
		end

		self:applyBrand(self.brandId)

		g_isPresentationVersionUseReloadButton = true
...
function GamingStationManager:load()
	...
	g_messageCenter:subscribe(MessageType.GUI_MAIN_SCREEN_OPEN, self.onOpenMainScreen, self)
	g_messageCenter:subscribe(MessageType.GUI_CHARACTER_CREATION_SCREEN_OPEN, self.onOpenCharacterCreationScreen, self)
end
...
function GamingStationManager:update(dt)
	if self.loadState == GamingStationManager.LOAD_GS and g_gui.currentGuiName == "MainScreen" then
		...
		if self.brandId ~= newBrandId then
	...
	if fileExists(GamingStationManager.BRAND_CONFIGURATION_FILE) then
		...
		for i = 0, numOfGamepads - 1 do
			...
			if gamepadName == GamingStationManager.GAMEPAD_NAME then
				g_logManager:info("Found Gaming Station with gamepad '%s'", gamepadName)
			...
			if GS_IS_CONSOLE_VERSION then
				g_logManager:info("Found Gaming Station config, and usual gamepad '%s'", gamepadName)
		...
		g_logManager:error("Unable to find GamingStation!")
...
function GamingStationManager:applyBrand(brandId)
	if brandId ~= nil then
		...
		if brand ~= nil then
			if brand.savegame ~= nil then
				if GS_IS_CONSOLE_VERSION then
					...
					if not fileExists(savegamePath .. "/careerSavegame.xml") then
						g_logManager:error("Failed to copy gamingStation savegame from '%s' to '%s'!", savegameSourcePath, savegamePath)
					end
				end
			...
			if brand.storeItems ~= nil then
				g_isPresentationVersionSpecialStore = true
				g_isPresentationVersionSpecialStorePath = brand.storeItems
			end

			g_logManager:info("Brand for Gaming Station: '%s'", brand.name)
		end
	else
		g_logManager:info("Could not find brand for Gaming Station! Do not touch the dip switches!")
	end
end
...
function GamingStationManager:loadBrandFromXML(xmlFile, key)
	...
	local storeItems = getXMLString(xmlFile, key .. ".storeItems#filename")
...
function GamingStationManager:onOpenCharacterCreationScreen()
