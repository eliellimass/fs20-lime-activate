...
function BrandColorManager:loadBrandColorFromXML(xmlFile, key, isBaseType)
	...
	if name ~= nil and value ~= nil then
		...
		g_logManager:warning("Failed to load BrandColor '%s' ", key)
	end
end
...
function BrandColorManager:addBrandColor(name, value, isBaseType)
	...
	local color = StringUtil.getVectorNFromString(value, 4)
...
function BrandColorManager:loadColorAndMaterialFromXML(configFileName, node, shaderParam, xmlFile, key, name, required)
	local valueStr = getXMLString(xmlFile, key .. "#" .. (name or "value"))

	if valueStr == nil then
		if required == nil or required == true then
			g_logManager:xmlWarning(configFileName, "Missing color value for '%s'", key)
		end

		return nil
	end

	local value = self:getBrandColorByName(valueStr)

	if value == nil then
		value = {
			StringUtil.getVectorFromString(valueStr, 3)
		}

		if #value < 3 then
			g_logManager:xmlWarning(configFileName, "Missing color values in '%s' for '%s'", valueStr, key)
		end
	end

	if value == nil then
		g_logManager:xmlWarning(configFileName, "Could not evaluate shader parameter value '%s' for '%s'", valueStr, key)

		return nil
	end

	local materialId = nil
	local materialStr = getXMLInt(xmlFile, key .. "#material")
