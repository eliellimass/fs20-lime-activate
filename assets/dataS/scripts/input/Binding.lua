...
Binding.PRESSED_MAGNITUDE_THRESHOLD = 0.5
...
function Binding.createFromXML(xmlFile, elementTag)
	...
	local axisNames = StringUtil.splitString(" ", axisNamesText)
...
function Binding:updateData(deviceId, deviceCategory, axisNames, inputComponent, isInit)
	...
	self.axisNameSet = ListUtil.listToSet(self.axisNames)
...
function Binding:hasCollisionWith(otherBinding)
	...
	local sameInput = ListUtil.areListsEqual(self.axisNames, otherBinding.axisNames, true)
...
function Binding.needJapanesePlaystationButtonSwap()
	return GS_IS_CONSOLE_VERSION and getGameTerritory() == "jp"
end
