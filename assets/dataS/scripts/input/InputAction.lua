...
function InputAction.new(name, categories, axisType, isLocked, ignoreComboMask, displayNamePositive, displayNameNegative, isBaseAction, isConsoleAction, isMobileAction)
...
function InputAction.createFromXML(xmlFile, elementTag)
	...
	local categoryNames = StringUtil.splitString(" ", categoryValue)
	local categories = {}
	...
	return InputAction.new(name, categories, axisType, isLocked, ignoreComboMask, nil, , isBaseAction, isConsoleAction, isMobileAction)
end

function InputAction:getIsSupportedOnCurrentPlatfrom()
	if not self.isBaseAction and GS_PLATFORM_TYPE == GS_PLATFORM_TYPE_PC or not self.isConsoleAction and GS_IS_CONSOLE_VERSION or not self.isMobileAction and GS_IS_MOBILE_VERSION then
		return false
	end
...
function InputAction:enableBinding(binding)
	for i, existingBinding in ipairs(self.bindings) do
		if existingBinding.id == binding.id then
			ListUtil.addElementToList(self.activeBindings, binding)
		end
	end
end
...
function InputAction:clone()
	local clone = InputAction.new(self.name, self.categories, self.axisType, self.isLocked, self.ignoreComboMask, self.displayNamePositive, self.displayNameNegative)
