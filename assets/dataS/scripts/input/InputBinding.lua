...
InputBinding.INPUT_MODE_CHANGE_MIN_INTERVAL = GS_IS_MOBILE_VERSION and 250 or 5000
...
function InputBinding:new(logManager, modManager, messageCenter, isConsoleVersion)
	...
	addConsoleCommand("gsEnableInputDebug", "", "consoleCommandEnableInputDebug", self)
	addConsoleCommand("gsPrintInputContext", "", "consoleCommandPrintInputContext", self)
...
function InputBinding:assignPlatformBindingPaths()
	...
	elseif GS_PLATFORM_TYPE == GS_PLATFORM_TYPE_IOS then
...
function InputBinding:validateActionEventParameters(actionName, targetObject, eventCallback, triggerUp, triggerDown, triggerAlways)
	...
	if InputAction[actionName] == nil then
		self.logManager:devWarning("Warning: Tried registering an event for an unknown action: %s", actionName)

		valid = false
	end

	if not targetObject then
		self.logManager:devWarning("Warning: Tried registering an action event without a target.")

		valid = false
	end

	if not eventCallback then
		self.logManager:devWarning("Warning: Tried registering an action event without an event callback.")

		valid = false
	end

	if not triggerUp and not triggerDown and not triggerAlways then
		self.logManager:devWarning("Warning: Tried registering an action event without any active trigger flags.")
...
function InputBinding:registerActionEvent(actionName, targetObject, eventCallback, triggerUp, triggerDown, triggerAlways, startActive, callbackState, disableConflictingBindings)
	...
	if startActive then
		local eventCollision, collidingAction = self:checkEventCollision(actionName, disableConflictingBindings)
...
function InputBinding:checkEventCollision(actionName, disableConflictingBindings)
	local testAction = self.nameActions[actionName]

	if testAction == nil then
		log("testActionName", actionName)
		printCallstack()
	end
	...
	for otherAction, events in pairs(actionEvents) do
		local isSameCategory = ListUtil.hasSetIntersection(testAction.categories, otherAction.categories)
	...
	if #disabledBindings > 0 then
		hasCollision = #testAction:getBindings() > 0 and #testAction:getActiveBindings() == 0
...
function InputBinding:beginActionEventsModification(inContextName, createNew)
	if inContextName == nil or inContextName == InputBinding.NO_REGISTRATION_CONTEXT.name then
		self.logManager:devWarning("Cannot begin action event registration with an empty context name.")
	...
	if context == nil or createNew then
		...
		self.contexts[inContextName] = context
	end

	if self.debugEnabled then
		self.logManager:devInfo("[InputBinding] Beginning action events modification in context [%s]", inContextName)
	end
...
function InputBinding:endActionEventsModification(ignoreCheck)
	if not ignoreCheck and self.registrationContext == InputBinding.NO_REGISTRATION_CONTEXT then
		self.logManager:devWarning("Called InputBinding:endActionEventsModification() when the registration context is already reset. Check call order.")

		return
	end

	if self.debugEnabled then
		self.logManager:devInfo("[InputBinding] Ended action events modification in context [%s]", self.registrationContext.name)
	end
...
function InputBinding:getComboActionNameForAxisSet(modifierAxisSet)
	for comboSet, comboActionName in pairs(self.comboInputActions) do
		if ListUtil.areSetsEqual(comboSet, modifierAxisSet) then
...
function InputBinding:assignLastInputHelpMode(inputHelpMode)
	local needCurrentInputModeNotification = self.lastInputMode ~= inputHelpMode
	...
	if inputHelpMode == GS_INPUT_HELP_MODE_KEYBOARD or InputBinding.INPUT_MODE_CHANGE_MIN_INTERVAL <= self.timeSinceLastInputHelpModeChange then
		local needInputHelpModeNotification = self.lastInputHelpMode ~= inputHelpMode
...
function InputBinding:rollbackBindingChanges()
	...
	for action, bindings in pairs(self.originalActionBindings) do
		for _, binding in pairs(bindings) do
			binding:makeId()
...
function InputBinding:loadActions(xmlFile, modName)
	...
	while true do
		...
		if action ~= nil then
			...
			local modPart = modName and " in " .. modName or ""
...
function InputBinding:validateAndRepairComboActionBindings()
	for _, action in pairs(self.actions) do
		...
		if comboAxisNames then
			local axisNames = ListUtil.copyTable(comboAxisNames)
...
function InputBinding:getBindingComboMask(binding)
	for comboSet, mask in pairs(self.comboInputAxisMasks) do
		if ListUtil.areSetsEqual(comboSet, binding.modifierAxisSet) then
			return mask
		end
	end
...
function InputBinding:addBinding(action, binding, silentIgnoreDuplicates)
	if not self:adjustBindingSlotIndex(binding, action) then
		if not silentIgnoreDuplicates then
			self.logManager:warning("Tried to add additional alternative binding %s to %s. The maximum alternative count has been reached. The new binding has been ignored.", tostring(binding), tostring(action.name))
		end
	...
	elseif hasDuplicate then
		if not silentIgnoreDuplicates then
			self.logManager:warning("Tried to add duplicate binding %s to %s. The new binding has been ignored.", tostring(binding), tostring(action.name))
		end
	elseif slotOccupied and not silentIgnoreDuplicates then
		self.logManager:warning("Tried assigning the binding %s to %s to an occupied slot. The new binding has been ignored", tostring(binding), tostring(action.name))
	end
...
function InputBinding:checkGamepadsChanged()
	...
	if numGamepads ~= self.numGamepads then
		...
		if binding.isActive then
			...
			for _, otherBinding in ipairs(deviceBindings) do
				if otherBinding ~= binding then
					...
					if ListUtil.isRealSubset(otherBinding.axisNameSet, binding.axisNameSet) then
						otherBinding.isShadowed = otherBinding.isShadowed or bindingActive
					elseif ListUtil.isRealSubset(binding.axisNameSet, otherBinding.axisNameSet) then
...
function InputBinding:setContext(name, createNew, deletePrevious)
	...
	if deletePrevious and self.currentContextName ~= InputBinding.ROOT_CONTEXT_NAME then
		self.contexts[self.currentContextName] = nil
	end

	if self.debugEnabled then
		self.logManager:devInfo("[InputBinding] Set input context from [%s] to [%s], createNew=%s, deletePrevious=%s", tostring(self.currentContextName), tostring(name), tostring(createNew or false), tostring(deletePrevious or false))
	end
...
function InputBinding:revertContext(deleteCurrent)
	if self.currentContextName == InputBinding.ROOT_CONTEXT_NAME then
		self.logManager:devWarning("Tried reverting input context when at root level.")
	...
	if prevContext then
		...
		if self.debugEnabled then
			self.logManager:devInfo("[InputBinding] Reverting input context from [%s] to [%s], deleteCurrent=%s", tostring(self.currentContextName), tostring(prevContextName), tostring(deleteCurrent or false))
		end
		...
		self.logManager:warning("Tried reverting to input context [%s] which is not defined.", tostring(prevContextName))
	end
end
...
function InputBinding:clearAllContexts()
	...
	if self.debugEnabled then
		self.logManager:devInfo("[InputBinding] Cleared all contexts")
	end
...
function InputBinding:checkSettingsIntegrity(inputBindingPath, inputBindingPathTemplate)
	...
	if isCorrupted then
		self.logManager:error("User input bindings corrupted. Replacing with defaults...")
	end
				...
				for _, binding1 in ipairs(action1.bindings) do
					for _, binding2 in ipairs(action2.bindings) do
						...
						if bothPrimary and sameInput then
							self.logManager:devError("Currently loaded input bindings have conflicting primary bindings in action group '%s' for actions [%s] and [%s], both bound to [%s]", groupId, action1.name, action2.name, binding2.inputString)
						end
					end
				end
			end
...
function InputBinding:consoleCommandEnableInputDebug(enable)
