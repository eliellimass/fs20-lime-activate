...
function GamepadHelper.getButtonAndAxisNames(buttonIdList, axisId, deviceId)
	...
	local deviceString = "('" .. getGamepadName(deviceId) .. "' " .. GamepadHelper.getDeviceString(deviceId) .. ")"
...
function GamepadHelper.getInputDisplayText(inputList, internalDeviceId)
	...
	return string.format("%s [%s]", table.concat(names, ", "), getGamepadName(internalDeviceId))
end
